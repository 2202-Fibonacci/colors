module.exports = {
  101: {
    complex: false,
    draw: true,
    stop_id: "101",
    stop_name: "Van Cortlandt Park - 242 St",
    stop_desc: "",
    stop_lat: "40.889248",
    stop_lon: "-73.898583",
    location_type: "1",
    lines_at: ["1"],
    borough: "bronx",
    accessible: false,
    north_label: "",
    south_label: "Manhattan",
  },
  103: {
    complex: false,
    draw: true,
    stop_id: "103",
    stop_name: "238 St",
    stop_desc: "",
    stop_lat: "40.884667",
    stop_lon: "-73.90087",
    location_type: "1",
    lines_at: ["1"],
    borough: "bronx",
    accessible: false,
    north_label: "242 St",
    south_label: "Manhattan",
  },
  104: {
    complex: false,
    draw: true,
    stop_id: "104",
    stop_name: "231 St",
    stop_desc: "",
    stop_lat: "40.878856",
    stop_lon: "-73.904834",
    location_type: "1",
    lines_at: ["1"],
    borough: "bronx",
    accessible: true,
    north_label: "242 St",
    south_label: "Manhattan",
  },
  106: {
    complex: false,
    draw: true,
    stop_id: "106",
    stop_name: "Marble Hill - 225 St",
    stop_desc: "",
    stop_lat: "40.874561",
    stop_lon: "-73.909831",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: false,
    north_label: "242 St",
    south_label: "Downtown",
  },
  107: {
    complex: false,
    draw: true,
    stop_id: "107",
    stop_name: "215 St",
    stop_desc: "",
    stop_lat: "40.869444",
    stop_lon: "-73.915279",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: false,
    north_label: "The Bronx",
    south_label: "Downtown",
  },
  108: {
    complex: false,
    draw: true,
    stop_id: "108",
    stop_name: "207 St",
    stop_desc: "",
    stop_lat: "40.864621",
    stop_lon: "-73.918822",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  109: {
    complex: false,
    draw: true,
    stop_id: "109",
    stop_name: "Dyckman St",
    stop_desc: "",
    stop_lat: "40.860531",
    stop_lon: "-73.925536",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  110: {
    complex: false,
    draw: true,
    stop_id: "110",
    stop_name: "191 St",
    stop_desc: "",
    stop_lat: "40.855225",
    stop_lon: "-73.929412",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  111: {
    complex: false,
    draw: true,
    stop_id: "111",
    stop_name: "181 St",
    stop_desc: "",
    stop_lat: "40.849505",
    stop_lon: "-73.933596",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  112: {
    complex: false,
    draw: true,
    stop_id: "112",
    stop_name: "168 St - Washington Hts",
    stop_desc: "",
    stop_lat: "40.840556",
    stop_lon: "-73.940133",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  113: {
    complex: false,
    draw: true,
    stop_id: "113",
    stop_name: "157 St",
    stop_desc: "",
    stop_lat: "40.834041",
    stop_lon: "-73.94489",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  114: {
    complex: false,
    draw: true,
    stop_id: "114",
    stop_name: "145 St",
    stop_desc: "",
    stop_lat: "40.826551",
    stop_lon: "-73.95036",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  115: {
    complex: false,
    draw: true,
    stop_id: "115",
    stop_name: "137 St - City College",
    stop_desc: "",
    stop_lat: "40.822008",
    stop_lon: "-73.953676",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  116: {
    complex: false,
    draw: true,
    stop_id: "116",
    stop_name: "125 St",
    stop_desc: "",
    stop_lat: "40.815581",
    stop_lon: "-73.958372",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  117: {
    complex: false,
    draw: true,
    stop_id: "117",
    stop_name: "116 St - Columbia University",
    stop_desc: "",
    stop_lat: "40.807722",
    stop_lon: "-73.96411",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  118: {
    complex: false,
    draw: true,
    stop_id: "118",
    stop_name: "Cathedral Pkwy",
    stop_desc: "",
    stop_lat: "40.803967",
    stop_lon: "-73.966847",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  119: {
    complex: false,
    draw: true,
    stop_id: "119",
    stop_name: "103 St",
    stop_desc: "",
    stop_lat: "40.799446",
    stop_lon: "-73.968379",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  120: {
    complex: false,
    draw: true,
    stop_id: "120",
    stop_name: "96 St",
    stop_desc: "",
    stop_lat: "40.793919",
    stop_lon: "-73.972323",
    location_type: "1",
    lines_at: ["1", "2", "3"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  121: {
    complex: false,
    draw: true,
    stop_id: "121",
    stop_name: "86 St",
    stop_desc: "",
    stop_lat: "40.788644",
    stop_lon: "-73.976218",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  122: {
    complex: false,
    draw: true,
    stop_id: "122",
    stop_name: "79 St",
    stop_desc: "",
    stop_lat: "40.783934",
    stop_lon: "-73.979917",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  123: {
    complex: false,
    draw: true,
    stop_id: "123",
    stop_name: "72 St",
    stop_desc: "",
    stop_lat: "40.778453",
    stop_lon: "-73.98197",
    location_type: "1",
    lines_at: ["1", "2", "3"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  124: {
    complex: false,
    draw: true,
    stop_id: "124",
    stop_name: "66 St - Lincoln Center",
    stop_desc: "",
    stop_lat: "40.77344",
    stop_lon: "-73.982209",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  125: {
    complex: false,
    draw: false,
    stop_id: "125",
    stop_name: "59 St - Columbus Circle",
    stop_desc: "",
    stop_lat: "40.768247",
    stop_lon: "-73.981929",
    location_type: "1",
    lines_at: ["1", "2", "3"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  126: {
    complex: false,
    draw: true,
    stop_id: "126",
    stop_name: "50 St",
    stop_desc: "",
    stop_lat: "40.761728",
    stop_lon: "-73.983849",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  127: {
    complex: false,
    draw: false,
    stop_id: "127",
    stop_name: "Times Sq - 42 St",
    stop_desc: "",
    stop_lat: "40.75529",
    stop_lon: "-73.987495",
    location_type: "1",
    lines_at: ["1", "2", "3"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  128: {
    complex: false,
    draw: true,
    stop_id: "128",
    stop_name: "34 St - Penn Station",
    stop_desc: "",
    stop_lat: "40.750373",
    stop_lon: "-73.991057",
    location_type: "1",
    lines_at: ["1", "2", "3"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  129: {
    complex: false,
    draw: true,
    stop_id: "129",
    stop_name: "28 St",
    stop_desc: "",
    stop_lat: "40.747215",
    stop_lon: "-73.993365",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  130: {
    complex: false,
    draw: true,
    stop_id: "130",
    stop_name: "23 St",
    stop_desc: "",
    stop_lat: "40.744081",
    stop_lon: "-73.995657",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  131: {
    complex: false,
    draw: true,
    stop_id: "131",
    stop_name: "18 St",
    stop_desc: "",
    stop_lat: "40.74104",
    stop_lon: "-73.997871",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  132: {
    complex: false,
    draw: true,
    stop_id: "132",
    stop_name: "14 St",
    stop_desc: "",
    stop_lat: "40.737826",
    stop_lon: "-74.000201",
    location_type: "1",
    lines_at: ["1", "2", "3"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  133: {
    complex: false,
    draw: true,
    stop_id: "133",
    stop_name: "Christopher St - Sheridan Sq",
    stop_desc: "",
    stop_lat: "40.733422",
    stop_lon: "-74.002906",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  134: {
    complex: false,
    draw: true,
    stop_id: "134",
    stop_name: "Houston St",
    stop_desc: "",
    stop_lat: "40.728251",
    stop_lon: "-74.005367",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  135: {
    complex: false,
    draw: true,
    stop_id: "135",
    stop_name: "Canal St",
    stop_desc: "",
    stop_lat: "40.722854",
    stop_lon: "-74.006277",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  136: {
    complex: false,
    draw: true,
    stop_id: "136",
    stop_name: "Franklin St",
    stop_desc: "",
    stop_lat: "40.719318",
    stop_lon: "-74.006886",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  137: {
    complex: false,
    draw: true,
    stop_id: "137",
    stop_name: "Chambers St",
    stop_desc: "",
    stop_lat: "40.715478",
    stop_lon: "-74.009266",
    location_type: "1",
    lines_at: ["1", "2", "3"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  138: {
    complex: false,
    draw: true,
    stop_id: "138",
    stop_name: "WTC Cortlandt St",
    stop_desc: "",
    stop_lat: "40.711835",
    stop_lon: "-74.012188",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  139: {
    complex: false,
    draw: true,
    stop_id: "139",
    stop_name: "Rector St",
    stop_desc: "",
    stop_lat: "40.707513",
    stop_lon: "-74.013783",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  // 140: {
  //  complex: false, //same as 142
  //   stop_id: "140",
  //   stop_name: "South Ferry Loop",
  //   stop_desc: "",
  //   stop_lat: "40.701411",
  //   stop_lon: "-74.013205",
  //   location_type: "1",
  //   lines_at: ["1"],
  //   transfers: ["R", "W"],
  //   borough: "manhattan",
  //   accessible: true,
  //   north_label: "Uptown & The Bronx",
  //   south_label: "",
  // },
  142: {
    complex: false, // same as 140
    draw: false,
    stop_id: "142",
    stop_name: "South Ferry",
    stop_desc: "",
    stop_lat: "40.702068",
    stop_lon: "-74.013664",
    location_type: "1",
    lines_at: ["1"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & The Bronx",
    south_label: "",
  },
  201: {
    complex: false,
    draw: true,
    stop_id: "201",
    stop_name: "Wakefield - 241 St",
    stop_desc: "",
    stop_lat: "40.903125",
    stop_lon: "-73.85062",
    location_type: "1",
    lines_at: ["2"],
    borough: "bronx",
    accessible: false,
    north_label: "",
    south_label: "Manhattan",
  },
  204: {
    complex: false,
    draw: true,
    stop_id: "204",
    stop_name: "Nereid Av",
    stop_desc: "",
    stop_lat: "40.898379",
    stop_lon: "-73.854376",
    location_type: "1",
    lines_at: ["2"],
    borough: "bronx",
    accessible: false,
    north_label: "Wakefield - 241 St",
    south_label: "Manhattan",
  },
  205: {
    complex: false,
    draw: true,
    stop_id: "205",
    stop_name: "233 St",
    stop_desc: "",
    stop_lat: "40.893193",
    stop_lon: "-73.857473",
    location_type: "1",
    lines_at: ["2"],
    borough: "bronx",
    accessible: true,
    north_label: "Wakefield - 241 St",
    south_label: "Manhattan",
  },
  206: {
    complex: false,
    draw: true,
    stop_id: "206",
    stop_name: "225 St",
    stop_desc: "",
    stop_lat: "40.888022",
    stop_lon: "-73.860341",
    location_type: "1",
    lines_at: ["2"],
    borough: "bronx",
    accessible: false,
    north_label: "Wakefield - 241 St",
    south_label: "Manhattan",
  },
  207: {
    complex: false,
    draw: true,
    stop_id: "207",
    stop_name: "219 St",
    stop_desc: "",
    stop_lat: "40.883895",
    stop_lon: "-73.862633",
    location_type: "1",
    lines_at: ["2"],
    borough: "bronx",
    accessible: false,
    north_label: "Wakefield - 241 St",
    south_label: "Manhattan",
  },
  208: {
    complex: false,
    draw: true,
    stop_id: "208",
    stop_name: "Gun Hill Rd",
    stop_desc: "",
    stop_lat: "40.87785",
    stop_lon: "-73.866256",
    location_type: "1",
    lines_at: ["2"],
    borough: "bronx",
    accessible: true,
    north_label: "Wakefield - 241 St",
    south_label: "Manhattan",
  },
  209: {
    complex: false,
    draw: true,
    stop_id: "209",
    stop_name: "Burke Av",
    stop_desc: "",
    stop_lat: "40.871356",
    stop_lon: "-73.867164",
    location_type: "1",
    lines_at: ["2"],
    borough: "bronx",
    accessible: false,
    north_label: "Wakefield - 241 St",
    south_label: "Manhattan",
  },
  210: {
    complex: false,
    draw: true,
    stop_id: "210",
    stop_name: "Allerton Av",
    stop_desc: "",
    stop_lat: "40.865462",
    stop_lon: "-73.867352",
    location_type: "1",
    lines_at: ["2"],
    borough: "bronx",
    accessible: false,
    north_label: "Wakefield - 241 St",
    south_label: "Manhattan",
  },
  211: {
    complex: false,
    draw: true,
    stop_id: "211",
    stop_name: "Pelham Pkwy",
    stop_desc: "",
    stop_lat: "40.857192",
    stop_lon: "-73.867615",
    location_type: "1",
    lines_at: ["2"],
    borough: "bronx",
    accessible: true,
    north_label: "Wakefield - 241 St",
    south_label: "Manhattan",
  },
  212: {
    complex: false,
    draw: true,
    stop_id: "212",
    stop_name: "Bronx Park East",
    stop_desc: "",
    stop_lat: "40.848828",
    stop_lon: "-73.868457",
    location_type: "1",
    lines_at: ["2"],
    borough: "bronx",
    accessible: false,
    north_label: "Wakefield - 241 St",
    south_label: "Manhattan",
  },
  213: {
    complex: false,
    draw: true,
    stop_id: "213",
    stop_name: "E 180 St",
    stop_desc: "",
    stop_lat: "40.841894",
    stop_lon: "-73.873488",
    location_type: "1",
    lines_at: ["2", "5"],
    borough: "bronx",
    accessible: true,
    north_label: "Wakefield - Eastchester",
    south_label: "Manhattan",
  },
  214: {
    complex: false,
    draw: true,
    stop_id: "214",
    stop_name: "West Farms Sq - E Tremont Av",
    stop_desc: "",
    stop_lat: "40.840295",
    stop_lon: "-73.880049",
    location_type: "1",
    lines_at: ["2", "5"],
    borough: "bronx",
    accessible: false,
    north_label: "Wakefield - Eastchester",
    south_label: "Manhattan",
  },
  215: {
    complex: false,
    draw: true,
    stop_id: "215",
    stop_name: "174 St",
    stop_desc: "",
    stop_lat: "40.837288",
    stop_lon: "-73.887734",
    location_type: "1",
    lines_at: ["2", "5"],
    borough: "bronx",
    accessible: false,
    north_label: "Wakefield - Eastchester",
    south_label: "Manhattan",
  },
  216: {
    complex: false,
    draw: true,
    stop_id: "216",
    stop_name: "Freeman St",
    stop_desc: "",
    stop_lat: "40.829993",
    stop_lon: "-73.891865",
    location_type: "1",
    lines_at: ["2", "5"],
    borough: "bronx",
    accessible: false,
    north_label: "Wakefield - Eastchester",
    south_label: "Manhattan",
  },
  217: {
    complex: false,
    draw: true,
    stop_id: "217",
    stop_name: "Simpson St",
    stop_desc: "",
    stop_lat: "40.824073",
    stop_lon: "-73.893064",
    location_type: "1",
    lines_at: ["2", "5"],
    borough: "bronx",
    accessible: true,
    north_label: "Wakefield - Eastchester",
    south_label: "Manhattan",
  },
  218: {
    complex: false,
    draw: true,
    stop_id: "218",
    stop_name: "Intervale Av",
    stop_desc: "",
    stop_lat: "40.822181",
    stop_lon: "-73.896736",
    location_type: "1",
    lines_at: ["2", "5"],
    borough: "bronx",
    accessible: false,
    north_label: "Wakefield - Eastchester",
    south_label: "Manhattan",
  },
  219: {
    complex: false,
    draw: true,
    stop_id: "219",
    stop_name: "Prospect Av",
    stop_desc: "",
    stop_lat: "40.819585",
    stop_lon: "-73.90177",
    location_type: "1",
    lines_at: ["2", "5"],
    borough: "bronx",
    accessible: false,
    north_label: "Wakefield - Eastchester",
    south_label: "Manhattan",
  },
  220: {
    complex: false,
    draw: true,
    stop_id: "220",
    stop_name: "Jackson Av",
    stop_desc: "",
    stop_lat: "40.81649",
    stop_lon: "-73.907807",
    location_type: "1",
    lines_at: ["2", "5"],
    borough: "bronx",
    accessible: false,
    north_label: "Wakefield - Eastchester",
    south_label: "Manhattan",
  },
  221: {
    complex: false,
    draw: true,
    stop_id: "221",
    stop_name: "3 Av - 149 St",
    stop_desc: "",
    stop_lat: "40.816109",
    stop_lon: "-73.917757",
    location_type: "1",
    lines_at: ["2", "5"],
    borough: "bronx",
    accessible: true,
    north_label: "Wakefield - Eastchester",
    south_label: "Manhattan",
  },
  222: {
    complex: false,
    draw: false,
    stop_id: "222",
    stop_name: "149 St - Grand Concourse",
    stop_desc: "",
    stop_lat: "40.81841",
    stop_lon: "-73.926718",
    location_type: "1",
    lines_at: ["2", "5"],
    borough: "bronx",
    accessible: false,
    north_label: "Wakefield - Eastchester",
    south_label: "Manhattan",
  },
  224: {
    complex: false,
    draw: true,
    stop_id: "224",
    stop_name: "135 St",
    stop_desc: "",
    stop_lat: "40.814229",
    stop_lon: "-73.94077",
    location_type: "1",
    lines_at: ["2", "3"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  225: {
    complex: false,
    draw: true,
    stop_id: "225",
    stop_name: "125 St",
    stop_desc: "",
    stop_lat: "40.807754",
    stop_lon: "-73.945495",
    location_type: "1",
    lines_at: ["2", "3"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  226: {
    complex: false,
    draw: true,
    stop_id: "226",
    stop_name: "116 St",
    stop_desc: "",
    stop_lat: "40.802098",
    stop_lon: "-73.949625",
    location_type: "1",
    lines_at: ["2", "3"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  227: {
    complex: false,
    draw: true,
    stop_id: "227",
    stop_name: "Central Park North (110 St)",
    stop_desc: "",
    stop_lat: "40.799075",
    stop_lon: "-73.951822",
    location_type: "1",
    lines_at: ["2", "3"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  228: {
    complex: false,
    draw: false,
    stop_id: "228",
    stop_name: "Park Pl",
    stop_desc: "",
    stop_lat: "40.713051",
    stop_lon: "-74.008811",
    location_type: "1",
    lines_at: ["2", "3"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  229: {
    complex: false,
    draw: false,
    stop_id: "229",
    stop_name: "Fulton St",
    stop_desc: "",
    stop_lat: "40.709416",
    stop_lon: "-74.006571",
    location_type: "1",
    lines_at: ["2", "3"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  230: {
    complex: false,
    draw: true,
    stop_id: "230",
    stop_name: "Wall St",
    stop_desc: "",
    stop_lat: "40.706821",
    stop_lon: "-74.0091",
    location_type: "1",
    lines_at: ["2", "3"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Brooklyn",
  },
  231: {
    complex: false,
    draw: true,
    stop_id: "231",
    stop_name: "Clark St",
    stop_desc: "",
    stop_lat: "40.697466",
    stop_lon: "-73.993086",
    location_type: "1",
    lines_at: ["2", "3"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Flatbush - New Lots",
  },
  232: {
    complex: false,
    draw: false,
    stop_id: "232",
    stop_name: "Borough Hall",
    stop_desc: "",
    stop_lat: "40.693219",
    stop_lon: "-73.989998",
    location_type: "1",
    lines_at: ["2", "3"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Flatbush - New Lots",
  },
  233: {
    complex: false,
    draw: true,
    stop_id: "233",
    stop_name: "Hoyt St",
    stop_desc: "",
    stop_lat: "40.690545",
    stop_lon: "-73.985065",
    location_type: "1",
    lines_at: ["2", "3"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Flatbush - New Lots",
  },
  234: {
    complex: false,
    draw: true,
    stop_id: "234",
    stop_name: "Nevins St",
    stop_desc: "",
    stop_lat: "40.688246",
    stop_lon: "-73.980492",
    location_type: "1",
    lines_at: ["2", "3", "4", "5"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Flatbush - Utica - New Lots",
  },
  235: {
    complex: false,
    draw: false,
    stop_id: "235",
    stop_name: "Atlantic Av - Barclays Ctr",
    stop_desc: "",
    stop_lat: "40.684359",
    stop_lon: "-73.977666",
    location_type: "1",
    lines_at: ["2", "3", "4", "5"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "Flatbush - New Lots",
  },
  236: {
    complex: false,
    draw: true,
    stop_id: "236",
    stop_name: "Bergen St",
    stop_desc: "",
    stop_lat: "40.680829",
    stop_lon: "-73.975098",
    location_type: "1",
    lines_at: ["2", "3"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Flatbush - New Lots",
  },
  237: {
    complex: false,
    draw: true,
    stop_id: "237",
    stop_name: "Grand Army Plaza",
    stop_desc: "",
    stop_lat: "40.675235",
    stop_lon: "-73.971046",
    location_type: "1",
    lines_at: ["2", "3"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Flatbush - New Lots",
  },
  238: {
    complex: false,
    draw: true,
    stop_id: "238",
    stop_name: "Eastern Pkwy - Brooklyn Museum",
    stop_desc: "",
    stop_lat: "40.671987",
    stop_lon: "-73.964375",
    location_type: "1",
    lines_at: ["2", "3"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "Flatbush - New Lots",
  },
  239: {
    complex: false,
    draw: true,
    stop_id: "239",
    stop_name: "Franklin Av - Medgar Evers College",
    stop_desc: "",
    stop_lat: "40.670682",
    stop_lon: "-73.958131",
    location_type: "1",
    lines_at: ["2", "3", "4", "5"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Flatbush - Utica - New Lots",
  },
  241: {
    complex: false,
    draw: true,
    stop_id: "241",
    stop_name: "President St - Medgar Evers College",
    stop_desc: "",
    stop_lat: "40.667883",
    stop_lon: "-73.950683",
    location_type: "1",
    lines_at: ["2", "5"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Flatbush",
  },
  242: {
    complex: false,
    draw: true,
    stop_id: "242",
    stop_name: "Sterling St",
    stop_desc: "",
    stop_lat: "40.662742",
    stop_lon: "-73.95085",
    location_type: "1",
    lines_at: ["2", "5"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Flatbush",
  },
  243: {
    complex: false,
    draw: true,
    stop_id: "243",
    stop_name: "Winthrop St",
    stop_desc: "",
    stop_lat: "40.656652",
    stop_lon: "-73.9502",
    location_type: "1",
    lines_at: ["2", "5"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Flatbush",
  },
  244: {
    complex: false,
    draw: true,
    stop_id: "244",
    stop_name: "Church Av",
    stop_desc: "",
    stop_lat: "40.650843",
    stop_lon: "-73.949575",
    location_type: "1",
    lines_at: ["2", "5"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "Flatbush",
  },
  245: {
    complex: false,
    draw: true,
    stop_id: "245",
    stop_name: "Beverly Rd",
    stop_desc: "",
    stop_lat: "40.645098",
    stop_lon: "-73.948959",
    location_type: "1",
    lines_at: ["2", "5"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Flatbush",
  },
  246: {
    complex: false,
    draw: true,
    stop_id: "246",
    stop_name: "Newkirk Av - Little Haiti",
    stop_desc: "",
    stop_lat: "40.639967",
    stop_lon: "-73.948411",
    location_type: "1",
    lines_at: ["2", "5"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Flatbush",
  },
  247: {
    complex: false,
    draw: true,
    stop_id: "247",
    stop_name: "Flatbush Av - Brooklyn College",
    stop_desc: "",
    stop_lat: "40.632836",
    stop_lon: "-73.947642",
    location_type: "1",
    lines_at: ["2", "5"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "",
  },
  248: {
    complex: false, // lumped in w/President-MEC
    stop_id: "248",
    stop_name: "Nostrand Av",
    stop_desc: "",
    stop_lat: "40.669847",
    stop_lon: "-73.950466",
    location_type: "1",
    lines_at: ["2", "5"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "New Lots",
  },
  249: {
    complex: false,
    draw: true,
    stop_id: "249",
    stop_name: "Kingston Av",
    stop_desc: "",
    stop_lat: "40.669399",
    stop_lon: "-73.942161",
    location_type: "1",
    lines_at: ["3"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "New Lots",
  },
  250: {
    complex: false,
    draw: true,
    stop_id: "250",
    stop_name: "Crown Hts - Utica Av",
    stop_desc: "",
    stop_lat: "40.668897",
    stop_lon: "-73.932942",
    location_type: "1",
    lines_at: ["3", "4", "5"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "New Lots",
  },
  251: {
    complex: false,
    draw: true,
    stop_id: "251",
    stop_name: "Sutter Av - Rutland Rd",
    stop_desc: "",
    stop_lat: "40.664717",
    stop_lon: "-73.92261",
    location_type: "1",
    lines_at: ["3"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "New Lots",
  },
  252: {
    complex: false,
    draw: true,
    stop_id: "252",
    stop_name: "Saratoga Av",
    stop_desc: "",
    stop_lat: "40.661453",
    stop_lon: "-73.916327",
    location_type: "1",
    lines_at: ["3"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "New Lots",
  },
  253: {
    complex: false,
    draw: true,
    stop_id: "253",
    stop_name: "Rockaway Av",
    stop_desc: "",
    stop_lat: "40.662549",
    stop_lon: "-73.908946",
    location_type: "1",
    lines_at: ["3"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "New Lots",
  },
  254: {
    complex: false,
    draw: true,
    stop_id: "254",
    stop_name: "Junius St",
    stop_desc: "",
    stop_lat: "40.663515",
    stop_lon: "-73.902447",
    location_type: "1",
    lines_at: ["3"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "New Lots",
  },
  255: {
    complex: false,
    draw: true,
    stop_id: "255",
    stop_name: "Pennsylvania Av",
    stop_desc: "",
    stop_lat: "40.664635",
    stop_lon: "-73.894895",
    location_type: "1",
    lines_at: ["3"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "New Lots",
  },
  256: {
    complex: false,
    draw: true,
    stop_id: "256",
    stop_name: "Van Siclen Av",
    stop_desc: "",
    stop_lat: "40.665449",
    stop_lon: "-73.889395",
    location_type: "1",
    lines_at: ["3"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "New Lots",
  },
  257: {
    complex: false,
    draw: true,
    stop_id: "257",
    stop_name: "New Lots Av",
    stop_desc: "",
    stop_lat: "40.666235",
    stop_lon: "-73.884079",
    location_type: "1",
    lines_at: ["3"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "",
  },
  301: {
    complex: false,
    draw: true,
    stop_id: "301",
    stop_name: "Harlem - 148 St",
    stop_desc: "",
    stop_lat: "40.82388",
    stop_lon: "-73.93647",
    location_type: "1",
    lines_at: ["3"],
    borough: "manhattan",
    accessible: false,
    north_label: "",
    south_label: "Downtown & Brooklyn",
  },
  302: {
    complex: false,
    draw: true,
    stop_id: "302",
    stop_name: "145 St",
    stop_desc: "",
    stop_lat: "40.820421",
    stop_lon: "-73.936245",
    location_type: "1",
    lines_at: ["3"],
    borough: "manhattan",
    accessible: false,
    north_label: "148 St",
    south_label: "Downtown & Brooklyn",
  },
  401: {
    complex: false,
    draw: true,
    stop_id: "401",
    stop_name: "Woodlawn",
    stop_desc: "",
    stop_lat: "40.886037",
    stop_lon: "-73.878751",
    location_type: "1",
    lines_at: ["4"],
    borough: "bronx",
    accessible: false,
    north_label: "",
    south_label: "Manhattan",
  },
  402: {
    complex: false,
    draw: true,
    stop_id: "402",
    stop_name: "Mosholu Pkwy",
    stop_desc: "",
    stop_lat: "40.87975",
    stop_lon: "-73.884655",
    location_type: "1",
    lines_at: ["4"],
    borough: "bronx",
    accessible: false,
    north_label: "Woodlawn",
    south_label: "Manhattan",
  },
  405: {
    complex: false,
    draw: true,
    stop_id: "405",
    stop_name: "Bedford Park Blvd - Lehman College",
    stop_desc: "",
    stop_lat: "40.873412",
    stop_lon: "-73.890064",
    location_type: "1",
    lines_at: ["4"],
    borough: "bronx",
    accessible: false,
    north_label: "Woodlawn",
    south_label: "Manhattan",
  },
  406: {
    complex: false,
    draw: true,
    stop_id: "406",
    stop_name: "Kingsbridge Rd",
    stop_desc: "",
    stop_lat: "40.86776",
    stop_lon: "-73.897174",
    location_type: "1",
    lines_at: ["4"],
    borough: "bronx",
    accessible: false,
    north_label: "Woodlawn",
    south_label: "Manhattan",
  },
  407: {
    complex: false,
    draw: true,
    stop_id: "407",
    stop_name: "Fordham Rd",
    stop_desc: "",
    stop_lat: "40.862803",
    stop_lon: "-73.901034",
    location_type: "1",
    lines_at: ["4"],
    borough: "bronx",
    accessible: true,
    north_label: "Woodlawn",
    south_label: "Manhattan",
  },
  408: {
    complex: false,
    draw: true,
    stop_id: "408",
    stop_name: "183 St",
    stop_desc: "",
    stop_lat: "40.858407",
    stop_lon: "-73.903879",
    location_type: "1",
    lines_at: ["4"],
    borough: "bronx",
    accessible: false,
    north_label: "Woodlawn",
    south_label: "Manhattan",
  },
  409: {
    complex: false,
    draw: true,
    stop_id: "409",
    stop_name: "Burnside Av",
    stop_desc: "",
    stop_lat: "40.853453",
    stop_lon: "-73.907684",
    location_type: "1",
    lines_at: ["4"],
    borough: "bronx",
    accessible: false,
    north_label: "Woodlawn",
    south_label: "Manhattan",
  },
  410: {
    complex: false,
    draw: true,
    stop_id: "410",
    stop_name: "176 St",
    stop_desc: "",
    stop_lat: "40.84848",
    stop_lon: "-73.911794",
    location_type: "1",
    lines_at: ["4"],
    borough: "bronx",
    accessible: false,
    north_label: "Woodlawn",
    south_label: "Manhattan",
  },
  411: {
    complex: false,
    draw: true,
    stop_id: "411",
    stop_name: "Mt Eden Av",
    stop_desc: "",
    stop_lat: "40.844434",
    stop_lon: "-73.914685",
    location_type: "1",
    lines_at: ["4"],
    borough: "bronx",
    accessible: false,
    north_label: "Woodlawn",
    south_label: "Manhattan",
  },
  412: {
    complex: false,
    draw: true,
    stop_id: "412",
    stop_name: "170 St",
    stop_desc: "",
    stop_lat: "40.840075",
    stop_lon: "-73.917791",
    location_type: "1",
    lines_at: ["4"],
    borough: "bronx",
    accessible: true,
    north_label: "Woodlawn",
    south_label: "Manhattan",
  },
  413: {
    complex: false,
    draw: true,
    stop_id: "413",
    stop_name: "167 St",
    stop_desc: "",
    stop_lat: "40.835537",
    stop_lon: "-73.9214",
    location_type: "1",
    lines_at: ["4"],
    borough: "bronx",
    accessible: false,
    north_label: "Woodlawn",
    south_label: "Manhattan",
  },
  414: {
    complex: false,
    draw: false,
    stop_id: "414",
    stop_name: "161 St - Yankee Stadium",
    stop_desc: "",
    stop_lat: "40.827994",
    stop_lon: "-73.925831",
    location_type: "1",
    lines_at: ["4"],
    borough: "bronx",
    accessible: true,
    north_label: "Woodlawn",
    south_label: "Manhattan",
  },
  415: {
    complex: false,
    draw: false,
    stop_id: "415",
    stop_name: "149 St - Grand Concourse",
    stop_desc: "",
    stop_lat: "40.818375",
    stop_lon: "-73.927351",
    location_type: "1",
    lines_at: ["4"],
    borough: "bronx",
    accessible: false,
    north_label: "Woodlawn",
    south_label: "Manhattan",
  },
  416: {
    complex: false,
    draw: true,
    stop_id: "416",
    stop_name: "138 St - Grand Concourse",
    stop_desc: "",
    stop_lat: "40.813224",
    stop_lon: "-73.929849",
    location_type: "1",
    lines_at: ["4", "5"],
    borough: "bronx",
    accessible: true,
    north_label: "Woodlawn",
    south_label: "Manhattan",
  },
  418: {
    complex: false,
    draw: false,
    stop_id: "418",
    stop_name: "Fulton St",
    stop_desc: "",
    stop_lat: "40.710368",
    stop_lon: "-74.009509",
    location_type: "1",
    lines_at: ["4", "5"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  419: {
    complex: false,
    draw: true,
    stop_id: "419",
    stop_name: "Wall St",
    stop_desc: "",
    stop_lat: "40.707557",
    stop_lon: "-74.011862",
    location_type: "1",
    lines_at: ["4", "5"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  420: {
    complex: false,
    draw: true,
    stop_id: "420",
    stop_name: "Bowling Green",
    stop_desc: "",
    stop_lat: "40.704817",
    stop_lon: "-74.014065",
    location_type: "1",
    lines_at: ["4", "5"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & The Bronx",
    south_label: "Brooklyn",
  },
  423: {
    complex: false,
    draw: false,
    stop_id: "423",
    stop_name: "Borough Hall",
    stop_desc: "",
    stop_lat: "40.692404",
    stop_lon: "-73.990151",
    location_type: "1",
    lines_at: ["4", "5"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Flatbush - Utica",
  },
  501: {
    complex: false,
    draw: true,
    stop_id: "501",
    stop_name: "Eastchester - Dyre Av",
    stop_desc: "",
    stop_lat: "40.8883",
    stop_lon: "-73.830834",
    location_type: "1",
    lines_at: ["5"],
    borough: "bronx",
    accessible: false,
    north_label: "",
    south_label: "Manhattan",
  },
  502: {
    complex: false,
    draw: true,
    stop_id: "502",
    stop_name: "Baychester Av",
    stop_desc: "",
    stop_lat: "40.878663",
    stop_lon: "-73.838591",
    location_type: "1",
    lines_at: ["5"],
    borough: "bronx",
    accessible: false,
    north_label: "Eastchester - Dyre Av",
    south_label: "Manhattan",
  },
  503: {
    complex: false,
    draw: true,
    stop_id: "503",
    stop_name: "Gun Hill Rd",
    stop_desc: "",
    stop_lat: "40.869526",
    stop_lon: "-73.846384",
    location_type: "1",
    lines_at: ["5"],
    borough: "bronx",
    accessible: true,
    north_label: "Eastchester - Dyre Av",
    south_label: "Manhattan",
  },
  504: {
    complex: false,
    draw: true,
    stop_id: "504",
    stop_name: "Pelham Pkwy",
    stop_desc: "",
    stop_lat: "40.858985",
    stop_lon: "-73.855359",
    location_type: "1",
    lines_at: ["5"],
    borough: "bronx",
    accessible: false,
    north_label: "Eastchester - Dyre Av",
    south_label: "Manhattan",
  },
  505: {
    complex: false,
    draw: true,
    stop_id: "505",
    stop_name: "Morris Park",
    stop_desc: "",
    stop_lat: "40.854364",
    stop_lon: "-73.860495",
    location_type: "1",
    lines_at: ["5"],
    borough: "bronx",
    accessible: false,
    north_label: "Eastchester - Dyre Av",
    south_label: "Manhattan",
  },
  601: {
    complex: false,
    draw: true,
    stop_id: "601",
    stop_name: "Pelham Bay Park",
    stop_desc: "",
    stop_lat: "40.852462",
    stop_lon: "-73.828121",
    location_type: "1",
    lines_at: ["6"],
    borough: "bronx",
    accessible: true,
    north_label: "",
    south_label: "Manhattan",
  },
  602: {
    complex: false,
    draw: true,
    stop_id: "602",
    stop_name: "Buhre Av",
    stop_desc: "",
    stop_lat: "40.84681",
    stop_lon: "-73.832569",
    location_type: "1",
    lines_at: ["6"],
    borough: "bronx",
    accessible: false,
    north_label: "Pelham Bay Park",
    south_label: "Manhattan",
  },
  603: {
    complex: false,
    draw: true,
    stop_id: "603",
    stop_name: "Middletown Rd",
    stop_desc: "",
    stop_lat: "40.843863",
    stop_lon: "-73.836322",
    location_type: "1",
    lines_at: ["6"],
    borough: "bronx",
    accessible: false,
    north_label: "Pelham Bay Park",
    south_label: "Manhattan",
  },
  604: {
    complex: false,
    draw: true,
    stop_id: "604",
    stop_name: "Westchester Sq - E Tremont Av",
    stop_desc: "",
    stop_lat: "40.839892",
    stop_lon: "-73.842952",
    location_type: "1",
    lines_at: ["6"],
    borough: "bronx",
    accessible: false,
    north_label: "Pelham Bay Park",
    south_label: "Manhattan",
  },
  606: {
    complex: false,
    draw: true,
    stop_id: "606",
    stop_name: "Zerega Av",
    stop_desc: "",
    stop_lat: "40.836488",
    stop_lon: "-73.847036",
    location_type: "1",
    lines_at: ["6"],
    borough: "bronx",
    accessible: false,
    north_label: "Pelham Bay Park",
    south_label: "Manhattan",
  },
  607: {
    complex: false,
    draw: true,
    stop_id: "607",
    stop_name: "Castle Hill Av",
    stop_desc: "",
    stop_lat: "40.834255",
    stop_lon: "-73.851222",
    location_type: "1",
    lines_at: ["6"],
    borough: "bronx",
    accessible: false,
    north_label: "Pelham Bay Park",
    south_label: "Manhattan",
  },
  608: {
    complex: false,
    draw: true,
    stop_id: "608",
    stop_name: "Parkchester",
    stop_desc: "",
    stop_lat: "40.833226",
    stop_lon: "-73.860816",
    location_type: "1",
    lines_at: ["6"],
    borough: "bronx",
    accessible: false,
    north_label: "Pelham Bay Park",
    south_label: "Manhattan",
  },
  609: {
    complex: false,
    draw: true,
    stop_id: "609",
    stop_name: "St Lawrence Av",
    stop_desc: "",
    stop_lat: "40.831509",
    stop_lon: "-73.867618",
    location_type: "1",
    lines_at: ["6"],
    borough: "bronx",
    accessible: false,
    north_label: "Pelham Bay Park",
    south_label: "Manhattan",
  },
  610: {
    complex: false,
    draw: true,
    stop_id: "610",
    stop_name: "Morrison Av- Sound View",
    stop_desc: "",
    stop_lat: "40.829521",
    stop_lon: "-73.874516",
    location_type: "1",
    lines_at: ["6"],
    borough: "bronx",
    accessible: false,
    north_label: "Pelham Bay Park",
    south_label: "Manhattan",
  },
  611: {
    complex: false,
    draw: true,
    stop_id: "611",
    stop_name: "Elder Av",
    stop_desc: "",
    stop_lat: "40.828584",
    stop_lon: "-73.879159",
    location_type: "1",
    lines_at: ["6"],
    borough: "bronx",
    accessible: false,
    north_label: "Pelham Bay Park",
    south_label: "Manhattan",
  },
  612: {
    complex: false,
    draw: true,
    stop_id: "612",
    stop_name: "Whitlock Av",
    stop_desc: "",
    stop_lat: "40.826525",
    stop_lon: "-73.886283",
    location_type: "1",
    lines_at: ["6"],
    borough: "bronx",
    accessible: false,
    north_label: "Pelham Bay Park",
    south_label: "Manhattan",
  },
  613: {
    complex: false,
    draw: true,
    stop_id: "613",
    stop_name: "Hunts Point Av",
    stop_desc: "",
    stop_lat: "40.820948",
    stop_lon: "-73.890549",
    location_type: "1",
    lines_at: ["6"],
    borough: "bronx",
    accessible: true,
    north_label: "Pelham Bay Park",
    south_label: "Manhattan",
  },
  614: {
    complex: false,
    draw: true,
    stop_id: "614",
    stop_name: "Longwood Av",
    stop_desc: "",
    stop_lat: "40.816104",
    stop_lon: "-73.896435",
    location_type: "1",
    lines_at: ["6"],
    borough: "bronx",
    accessible: false,
    north_label: "Pelham Bay Park",
    south_label: "Manhattan",
  },
  615: {
    complex: false,
    draw: true,
    stop_id: "615",
    stop_name: "E 149 St",
    stop_desc: "",
    stop_lat: "40.812118",
    stop_lon: "-73.904098",
    location_type: "1",
    lines_at: ["6"],
    borough: "bronx",
    accessible: false,
    north_label: "Pelham Bay Park",
    south_label: "Manhattan",
  },
  616: {
    complex: false,
    draw: true,
    stop_id: "616",
    stop_name: "E 143 St - St Mary's St",
    stop_desc: "",
    stop_lat: "40.808719",
    stop_lon: "-73.907657",
    location_type: "1",
    lines_at: ["6"],
    borough: "bronx",
    accessible: false,
    north_label: "Pelham Bay Park",
    south_label: "Manhattan",
  },
  617: {
    complex: false,
    draw: true,
    stop_id: "617",
    stop_name: "Cypress Av",
    stop_desc: "",
    stop_lat: "40.805368",
    stop_lon: "-73.914042",
    location_type: "1",
    lines_at: ["6"],
    borough: "bronx",
    accessible: false,
    north_label: "Pelham Bay Park",
    south_label: "Manhattan",
  },
  618: {
    complex: false,
    draw: true,
    stop_id: "618",
    stop_name: "Brook Av",
    stop_desc: "",
    stop_lat: "40.807566",
    stop_lon: "-73.91924",
    location_type: "1",
    lines_at: ["6"],
    borough: "bronx",
    accessible: false,
    north_label: "Pelham Bay Park",
    south_label: "Manhattan",
  },
  619: {
    complex: false,
    draw: true,
    stop_id: "619",
    stop_name: "3 Av - 138 St",
    stop_desc: "",
    stop_lat: "40.810476",
    stop_lon: "-73.926138",
    location_type: "1",
    lines_at: ["6"],
    borough: "bronx",
    accessible: false,
    north_label: "Pelham Bay Park",
    south_label: "Manhattan",
  },
  621: {
    complex: false,
    draw: true,
    stop_id: "621",
    stop_name: "125 St",
    stop_desc: "",
    stop_lat: "40.804138",
    stop_lon: "-73.937594",
    location_type: "1",
    lines_at: ["4", "5", "6"],
    borough: "manhattan",
    accessible: true,
    north_label: "The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  622: {
    complex: false,
    draw: true,
    stop_id: "622",
    stop_name: "116 St",
    stop_desc: "",
    stop_lat: "40.798629",
    stop_lon: "-73.941617",
    location_type: "1",
    lines_at: ["6"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  623: {
    complex: false,
    draw: true,
    stop_id: "623",
    stop_name: "110 St",
    stop_desc: "",
    stop_lat: "40.79502",
    stop_lon: "-73.94425",
    location_type: "1",
    lines_at: ["6"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  624: {
    complex: false,
    draw: true,
    stop_id: "624",
    stop_name: "103 St",
    stop_desc: "",
    stop_lat: "40.7906",
    stop_lon: "-73.947478",
    location_type: "1",
    lines_at: ["6"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  625: {
    complex: false,
    draw: true,
    stop_id: "625",
    stop_name: "96 St",
    stop_desc: "",
    stop_lat: "40.785672",
    stop_lon: "-73.95107",
    location_type: "1",
    lines_at: ["6"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  626: {
    complex: false,
    draw: true,
    stop_id: "626",
    stop_name: "86 St",
    stop_desc: "",
    stop_lat: "40.779492",
    stop_lon: "-73.955589",
    location_type: "1",
    lines_at: ["6"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  627: {
    complex: false,
    draw: true,
    stop_id: "627",
    stop_name: "77 St",
    stop_desc: "",
    stop_lat: "40.77362",
    stop_lon: "-73.959874",
    location_type: "1",
    lines_at: ["6"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  628: {
    complex: false,
    draw: true,
    stop_id: "628",
    stop_name: "68 St - Hunter College",
    stop_desc: "",
    stop_lat: "40.768141",
    stop_lon: "-73.96387",
    location_type: "1",
    lines_at: ["6"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  629: {
    complex: false,
    draw: false,
    stop_id: "629",
    stop_name: "59 St",
    stop_desc: "",
    stop_lat: "40.762526",
    stop_lon: "-73.967967",
    location_type: "1",
    lines_at: ["4", "5", "6"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  630: {
    complex: false, 
    draw: false,
    stop_id: "630",
    stop_name: "51 St",
    stop_desc: "",
    stop_lat: "40.757107",
    stop_lon: "-73.97192",
    location_type: "1",
    lines_at: ["6"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  631: {
    complex: false,
    draw: false,
    stop_id: "631",
    stop_name: "Grand Central - 42 St",
    stop_desc: "",
    stop_lat: "40.751776",
    stop_lon: "-73.976848",
    location_type: "1",
    lines_at: ["4", "5", "6"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  632: {
    complex: false,
    draw: true,
    stop_id: "632",
    stop_name: "33 St",
    stop_desc: "",
    stop_lat: "40.746081",
    stop_lon: "-73.982076",
    location_type: "1",
    lines_at: ["6"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  633: {
    complex: false,
    draw: true,
    stop_id: "633",
    stop_name: "28 St",
    stop_desc: "",
    stop_lat: "40.74307",
    stop_lon: "-73.984264",
    location_type: "1",
    lines_at: ["6"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  634: {
    complex: false,
    draw: true,
    stop_id: "634",
    stop_name: "23 St",
    stop_desc: "",
    stop_lat: "40.739864",
    stop_lon: "-73.986599",
    location_type: "1",
    lines_at: ["6"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  635: {
    complex: false,
    draw: false,
    stop_id: "635",
    stop_name: "14 St - Union Sq",
    stop_desc: "",
    stop_lat: "40.734673",
    stop_lon: "-73.989951",
    location_type: "1",
    lines_at: ["4", "5", "6"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  636: {
    complex: false,
    draw: true,
    stop_id: "636",
    stop_name: "Astor Pl",
    stop_desc: "",
    stop_lat: "40.730054",
    stop_lon: "-73.99107",
    location_type: "1",
    lines_at: ["6"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  637: {
    complex: false,
    draw: false,
    stop_id: "637",
    stop_name: "Bleecker St",
    stop_desc: "",
    stop_lat: "40.725915",
    stop_lon: "-73.994659",
    location_type: "1",
    lines_at: ["6"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  638: {
    complex: false,
    draw: true,
    stop_id: "638",
    stop_name: "Spring St",
    stop_desc: "",
    stop_lat: "40.722301",
    stop_lon: "-73.997141",
    location_type: "1",
    lines_at: ["6"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  639: {
    complex: false,
    draw: false,
    stop_id: "639",
    stop_name: "Canal St",
    stop_desc: "",
    stop_lat: "40.718803",
    stop_lon: "-74.000193",
    location_type: "1",
    lines_at: ["6"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown",
  },
  640: {
    complex: false,
    draw: false,
    stop_id: "640",
    stop_name: "Brooklyn Bridge - City Hall",
    stop_desc: "",
    stop_lat: "40.713065",
    stop_lon: "-74.004131",
    location_type: "1",
    lines_at: ["4", "5", "6"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  701: {
    complex: false,
    draw: true,
    stop_id: "701",
    stop_name: "Flushing - Main St",
    stop_desc: "",
    stop_lat: "40.7596",
    stop_lon: "-73.83003",
    location_type: "1",
    lines_at: ["7"],
    borough: "queens",
    accessible: true,
    north_label: "",
    south_label: "Manhattan",
  },
  702: {
    complex: false,
    draw: true,
    stop_id: "702",
    stop_name: "Mets - Willets Point",
    stop_desc: "",
    stop_lat: "40.754622",
    stop_lon: "-73.845625",
    location_type: "1",
    lines_at: ["7"],
    borough: "queens",
    accessible: false,
    north_label: "Flushing",
    south_label: "Manhattan",
  },
  705: {
    complex: false,
    draw: true,
    stop_id: "705",
    stop_name: "111 St",
    stop_desc: "",
    stop_lat: "40.75173",
    stop_lon: "-73.855334",
    location_type: "1",
    lines_at: ["7"],
    borough: "queens",
    accessible: false,
    north_label: "Flushing",
    south_label: "Manhattan",
  },
  706: {
    complex: false,
    draw: true,
    stop_id: "706",
    stop_name: "103 St - Corona Plaza",
    stop_desc: "",
    stop_lat: "40.749865",
    stop_lon: "-73.8627",
    location_type: "1",
    lines_at: ["7"],
    borough: "queens",
    accessible: false,
    north_label: "Flushing",
    south_label: "Manhattan",
  },
  707: {
    complex: false,
    draw: true,
    stop_id: "707",
    stop_name: "Junction Blvd",
    stop_desc: "",
    stop_lat: "40.749145",
    stop_lon: "-73.869527",
    location_type: "1",
    lines_at: ["7"],
    borough: "queens",
    accessible: true,
    north_label: "Flushing",
    south_label: "Manhattan",
  },
  708: {
    complex: false,
    draw: true,
    stop_id: "708",
    stop_name: "90 St - Elmhurst Av",
    stop_desc: "",
    stop_lat: "40.748408",
    stop_lon: "-73.876613",
    location_type: "1",
    lines_at: ["7"],
    borough: "queens",
    accessible: false,
    north_label: "Flushing",
    south_label: "Manhattan",
  },
  709: {
    complex: false,
    draw: true,
    stop_id: "709",
    stop_name: "82 St - Jackson Hts",
    stop_desc: "",
    stop_lat: "40.747659",
    stop_lon: "-73.883697",
    location_type: "1",
    lines_at: ["7"],
    borough: "queens",
    accessible: false,
    north_label: "Flushing",
    south_label: "Manhattan",
  },
  710: {
    complex: false,
    draw: false,
    stop_id: "710",
    stop_name: "74 St - Broadway",
    stop_desc: "",
    stop_lat: "40.746848",
    stop_lon: "-73.891394",
    location_type: "1",
    lines_at: ["7"],
    borough: "queens",
    accessible: true,
    north_label: "Flushing",
    south_label: "Manhattan",
  },
  711: {
    complex: false,
    draw: true,
    stop_id: "711",
    stop_name: "69 St",
    stop_desc: "",
    stop_lat: "40.746325",
    stop_lon: "-73.896403",
    location_type: "1",
    lines_at: ["7"],
    borough: "queens",
    accessible: false,
    north_label: "Flushing",
    south_label: "Manhattan",
  },
  712: {
    complex: false,
    draw: true,
    stop_id: "712",
    stop_name: "Woodside - 61 St",
    stop_desc: "",
    stop_lat: "40.74563",
    stop_lon: "-73.902984",
    location_type: "1",
    lines_at: ["7"],
    borough: "queens",
    accessible: true,
    north_label: "Flushing",
    south_label: "Manhattan",
  },
  713: {
    complex: false,
    draw: true,
    stop_id: "713",
    stop_name: "52 St",
    stop_desc: "",
    stop_lat: "40.744149",
    stop_lon: "-73.912549",
    location_type: "1",
    lines_at: ["7"],
    borough: "queens",
    accessible: false,
    north_label: "Flushing",
    south_label: "Manhattan",
  },
  714: {
    complex: false,
    draw: true,
    stop_id: "714",
    stop_name: "46 St - Bliss St",
    stop_desc: "",
    stop_lat: "40.743132",
    stop_lon: "-73.918435",
    location_type: "1",
    lines_at: ["7"],
    borough: "queens",
    accessible: false,
    north_label: "Flushing",
    south_label: "Manhattan",
  },
  715: {
    complex: false,
    draw: true,
    stop_id: "715",
    stop_name: "40 St - Lowery St",
    stop_desc: "",
    stop_lat: "40.743781",
    stop_lon: "-73.924016",
    location_type: "1",
    lines_at: ["7"],
    borough: "queens",
    accessible: false,
    north_label: "Flushing",
    south_label: "Manhattan",
  },
  716: {
    complex: false,
    draw: true,
    stop_id: "716",
    stop_name: "33 St - Rawson St",
    stop_desc: "",
    stop_lat: "40.744587",
    stop_lon: "-73.930997",
    location_type: "1",
    lines_at: ["7"],
    borough: "queens",
    accessible: false,
    north_label: "Flushing",
    south_label: "Manhattan",
  },
  718: {
    complex: false,
    draw: true,
    stop_id: "718",
    stop_name: "Queensboro Plaza",
    stop_desc: "",
    stop_lat: "40.750582",
    stop_lon: "-73.940202",
    location_type: "1",
    lines_at: ["7", "N", "W"],
    borough: "queens",
    accessible: false,
    north_label: "Astoria - Flushing",
    south_label: "Manhattan",
  },
  719: {
    complex: false,
    draw: false,
    stop_id: "719",
    stop_name: "Court Sq",
    stop_desc: "",
    stop_lat: "40.747023",
    stop_lon: "-73.945264",
    location_type: "1",
    lines_at: ["7"],
    transfers: ["G", "E", "M"],
    borough: "queens",
    accessible: false,
    north_label: "Flushing",
    south_label: "Manhattan",
  },
  720: {
    complex: false,
    draw: true,
    stop_id: "720",
    stop_name: "Hunters Point Av",
    stop_desc: "",
    stop_lat: "40.742216",
    stop_lon: "-73.948916",
    location_type: "1",
    lines_at: ["7"],
    borough: "queens",
    accessible: false,
    north_label: "Flushing",
    south_label: "Manhattan",
  },
  721: {
    complex: false,
    draw: true,
    stop_id: "721",
    stop_name: "Vernon Blvd - Jackson Av",
    stop_desc: "",
    stop_lat: "40.742626",
    stop_lon: "-73.953581",
    location_type: "1",
    lines_at: ["7"],
    borough: "queens",
    accessible: false,
    north_label: "Flushing",
    south_label: "Manhattan",
  },
  723: {
    complex: false,
    draw: false,
    stop_id: "723",
    stop_name: "Grand Central - 42 St",
    stop_desc: "",
    stop_lat: "40.751431",
    stop_lon: "-73.976041",
    location_type: "1",
    lines_at: ["7"],
    borough: "manhattan",
    accessible: true,
    north_label: "Queens",
    south_label: "34 St - Hudson Yards",
  },
  724: {
    complex: false,
    draw: false,
    stop_id: "724",
    stop_name: "5 Av",
    stop_desc: "",
    stop_lat: "40.753821",
    stop_lon: "-73.981963",
    location_type: "1",
    lines_at: ["7"],
    borough: "manhattan",
    accessible: false,
    north_label: "Queens",
    south_label: "34 St - Hudson Yards",
  },
  725: {
    complex: false,
    draw: false,
    stop_id: "725",
    stop_name: "Times Sq - 42 St",
    stop_desc: "",
    stop_lat: "40.755477",
    stop_lon: "-73.987691",
    location_type: "1",
    lines_at: ["7"],
    borough: "manhattan",
    accessible: true,
    north_label: "Queens",
    south_label: "34 St - Hudson Yards",
  },
  726: {
    complex: false,
    draw: true,
    stop_id: "726",
    stop_name: "34 St - Hudson Yards",
    stop_desc: "",
    stop_lat: "40.755882",
    stop_lon: "-74.001910",
    location_type: "1",
    lines_at: ["7"],
    borough: "manhattan",
    accessible: true,
    north_label: "Queens",
    south_label: "",
  },
  901: { // 42 St Shuttle
    complex: false,
    draw: false,
    stop_id: "901",
    stop_name: "Grand Central - 42 St",
    stop_desc: "",
    stop_lat: "40.752769",
    stop_lon: "-73.979189",
    location_type: "1",
    lines_at: ["S"],
    borough: "manhattan",
    accessible: true,
    north_label: "Times Sq",
    south_label: "",
  },
  902: {
    complex: false,
    draw: false,
    stop_id: "902",
    stop_name: "Times Sq - 42 St",
    stop_desc: "",
    stop_lat: "40.755983",
    stop_lon: "-73.986229",
    location_type: "1",
    lines_at: ["S"],
    borough: "manhattan",
    accessible: true,
    north_label: "",
    south_label: "Grand Central",
  },
  A02: {
    complex: false,
    draw: true,
    stop_id: "A02",
    stop_name: "Inwood - 207 St",
    stop_desc: "",
    stop_lat: "40.868072",
    stop_lon: "-73.919899",
    location_type: "1",
    lines_at: ["A"],
    borough: "manhattan",
    accessible: true,
    north_label: "",
    south_label: "Downtown & Brooklyn",
  },
  A03: {
    complex: false,
    draw: true,
    stop_id: "A03",
    stop_name: "Dyckman St",
    stop_desc: "",
    stop_lat: "40.865491",
    stop_lon: "-73.927271",
    location_type: "1",
    lines_at: ["A"],
    borough: "manhattan",
    accessible: false,
    north_label: "207 St",
    south_label: "Downtown & Brooklyn",
  },
  A05: {
    complex: false,
    draw: true,
    stop_id: "A05",
    stop_name: "190 St",
    stop_desc: "",
    stop_lat: "40.859022",
    stop_lon: "-73.93418",
    location_type: "1",
    lines_at: ["A"],
    borough: "manhattan",
    accessible: false,
    north_label: "207 St",
    south_label: "Downtown & Brooklyn",
  },
  A06: {
    complex: false,
    draw: true,
    stop_id: "A06",
    stop_name: "181 St",
    stop_desc: "",
    stop_lat: "40.851695",
    stop_lon: "-73.937969",
    location_type: "1",
    lines_at: ["A"],
    borough: "manhattan",
    accessible: false,
    north_label: "207 St",
    south_label: "Downtown & Brooklyn",
  },
  A07: {
    complex: false,
    draw: true,
    stop_id: "A07",
    stop_name: "175 St",
    stop_desc: "",
    stop_lat: "40.847391",
    stop_lon: "-73.939704",
    location_type: "1",
    lines_at: ["A"],
    borough: "manhattan",
    accessible: true,
    north_label: "207 St",
    south_label: "Downtown & Brooklyn",
  },
  A09: {
    complex: false,
    draw: true,
    stop_id: "A09",
    stop_name: "168 St",
    stop_desc: "",
    stop_lat: "40.840719",
    stop_lon: "-73.939561",
    location_type: "1",
    lines_at: ["A", "C"],
    borough: "manhattan",
    accessible: true,
    north_label: "207 St",
    south_label: "Downtown & Brooklyn",
  },
  A10: {
    complex: false,
    draw: true,
    stop_id: "A10",
    stop_name: "163 St - Amsterdam Av",
    stop_desc: "",
    stop_lat: "40.836013",
    stop_lon: "-73.939892",
    location_type: "1",
    lines_at: ["C"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown",
    south_label: "Downtown & Brooklyn",
  },
  A11: {
    complex: false,
    draw: true,
    stop_id: "A11",
    stop_name: "155 St",
    stop_desc: "",
    stop_lat: "40.830518",
    stop_lon: "-73.941514",
    location_type: "1",
    lines_at: ["C"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown",
    south_label: "Downtown & Brooklyn",
  },
  A12: {
    complex: false,
    draw: true,
    stop_id: "A12",
    stop_name: "145 St",
    stop_desc: "",
    stop_lat: "40.824783",
    stop_lon: "-73.944216",
    location_type: "1",
    lines_at: ["A", "B", "C", "D"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  A15: {
    complex: false,
    draw: true,
    stop_id: "A15",
    stop_name: "125 St",
    stop_desc: "",
    stop_lat: "40.811109",
    stop_lon: "-73.952343",
    location_type: "1",
    lines_at: ["A", "B", "C", "D"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  A16: {
    complex: false,
    draw: true,
    stop_id: "A16",
    stop_name: "116 St",
    stop_desc: "",
    stop_lat: "40.805085",
    stop_lon: "-73.954882",
    location_type: "1",
    lines_at: ["B", "C"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  A17: {
    complex: false,
    draw: true,
    stop_id: "A17",
    stop_name: "Cathedral Pkwy (110 St)",
    stop_desc: "",
    stop_lat: "40.800603",
    stop_lon: "-73.958161",
    location_type: "1",
    lines_at: ["B", "C"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  A18: {
    complex: false,
    draw: true,
    stop_id: "A18",
    stop_name: "103 St",
    stop_desc: "",
    stop_lat: "40.796092",
    stop_lon: "-73.961454",
    location_type: "1",
    lines_at: ["B", "C"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  A19: {
    complex: false,
    draw: true,
    stop_id: "A19",
    stop_name: "96 St",
    stop_desc: "",
    stop_lat: "40.791642",
    stop_lon: "-73.964696",
    location_type: "1",
    lines_at: ["B", "C"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  A20: {
    complex: false,
    draw: true,
    stop_id: "A20",
    stop_name: "86 St",
    stop_desc: "",
    stop_lat: "40.785868",
    stop_lon: "-73.968916",
    location_type: "1",
    lines_at: ["B", "C"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  A21: {
    complex: false,
    draw: true,
    stop_id: "A21",
    stop_name: "81 St - Museum of Natural History",
    stop_desc: "",
    stop_lat: "40.781433",
    stop_lon: "-73.972143",
    location_type: "1",
    lines_at: ["B", "C"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  A22: {
    complex: false,
    draw: true,
    stop_id: "A22",
    stop_name: "72 St",
    stop_desc: "",
    stop_lat: "40.775594",
    stop_lon: "-73.97641",
    location_type: "1",
    lines_at: ["B", "C"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  A24: {
    complex: false,
    draw: false,
    stop_id: "A24",
    stop_name: "59 St - Columbus Circle",
    stop_desc: "",
    stop_lat: "40.768296",
    stop_lon: "-73.981736",
    location_type: "1",
    lines_at: ["A", "C", "B", "D"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  A25: {
    complex: false,
    draw: true,
    stop_id: "A25",
    stop_name: "50 St",
    stop_desc: "",
    stop_lat: "40.762456",
    stop_lon: "-73.985984",
    location_type: "1",
    lines_at: ["C", "E"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown - Queens",
    south_label: "Downtown & Brooklyn",
  },
  A27: {
    complex: false,
    draw: false,
    stop_id: "A27",
    stop_name: "42 St - Port Authority Bus Terminal",
    stop_desc: "",
    stop_lat: "40.757308",
    stop_lon: "-73.989735",
    location_type: "1",
    lines_at: ["A", "C", "E"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown - Queens",
    south_label: "Downtown & Brooklyn",
  },
  A28: {
    complex: false,
    draw: true,
    stop_id: "A28",
    stop_name: "34 St - Penn Station",
    stop_desc: "",
    stop_lat: "40.752287",
    stop_lon: "-73.993391",
    location_type: "1",
    lines_at: ["A", "C", "E"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown - Queens",
    south_label: "Downtown & Brooklyn",
  },
  A30: {
    complex: false,
    draw: true,
    stop_id: "A30",
    stop_name: "23 St",
    stop_desc: "",
    stop_lat: "40.745906",
    stop_lon: "-73.998041",
    location_type: "1",
    lines_at: ["C", "E"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown - Queens",
    south_label: "Downtown & Brooklyn",
  },
  A31: {
    complex: false,
    draw: false,
    stop_id: "A31",
    stop_name: "14 St",
    stop_desc: "",
    stop_lat: "40.740893",
    stop_lon: "-74.00169",
    location_type: "1",
    lines_at: ["A", "C", "E"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown - Queens",
    south_label: "Downtown & Brooklyn",
  },
  A32: {
    complex: false,
    draw: true,
    stop_id: "A32",
    stop_name: "W 4 St - Washington Sq",
    stop_desc: "",
    stop_lat: "40.732338",
    stop_lon: "-74.000495",
    location_type: "1",
    lines_at: ["A", "C", "E"],
    transfers: ["B", "D", "F", "M"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown - Queens",
    south_label: "Downtown & Brooklyn",
  },
  A33: {
    complex: false,
    draw: true,
    stop_id: "A33",
    stop_name: "Spring St",
    stop_desc: "",
    stop_lat: "40.726227",
    stop_lon: "-74.003739",
    location_type: "1",
    lines_at: ["C", "E"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown - Queens",
    south_label: "Downtown & Brooklyn",
  },
  A34: {
    complex: false,
    draw: true,
    stop_id: "A34",
    stop_name: "Canal St",
    stop_desc: "",
    stop_lat: "40.720824",
    stop_lon: "-74.005229",
    location_type: "1",
    lines_at: ["A", "C", "E"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown - Queens",
    south_label: "Downtown & Brooklyn",
  },
  A36: {
    complex: false,
    draw: false,
    stop_id: "A36",
    stop_name: "Chambers St",
    stop_desc: "",
    stop_lat: "40.714111",
    stop_lon: "-74.008585",
    location_type: "1",
    lines_at: ["A", "C"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown",
    south_label: "Downtown & Brooklyn",
  },
  A38: {
    complex: false,
    draw: false,
    stop_id: "A38",
    stop_name: "Fulton St",
    stop_desc: "",
    stop_lat: "40.710197",
    stop_lon: "-74.007691",
    location_type: "1",
    lines_at: ["A", "C"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown",
    south_label: "Brooklyn",
  },
  A40: {
    complex: false,
    draw: true,
    stop_id: "A40",
    stop_name: "High St",
    stop_desc: "",
    stop_lat: "40.699337",
    stop_lon: "-73.990531",
    location_type: "1",
    lines_at: ["A", "C"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Lefferts Blvd - Rockaways",
  },
  A41: {
    complex: false,
    draw: false,
    stop_id: "A41",
    stop_name: "Jay St - MetroTech",
    stop_desc: "",
    stop_lat: "40.692338",
    stop_lon: "-73.987342",
    location_type: "1",
    lines_at: ["A", "C", "F"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "Lefferts Blvd - Rockaways",
  },
  A42: {
    complex: false,
    draw: true,
    stop_id: "A42",
    stop_name: "Hoyt - Schermerhorn Sts",
    stop_desc: "",
    stop_lat: "40.688484",
    stop_lon: "-73.985001",
    location_type: "1",
    lines_at: ["A", "C", "G"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan - Church Av",
    south_label: "Far Rockaway - Mott Av",
  },
  A43: {
    complex: false,
    draw: true,
    stop_id: "A43",
    stop_name: "Lafayette Av",
    stop_desc: "",
    stop_lat: "40.686113",
    stop_lon: "-73.973946",
    location_type: "1",
    lines_at: ["C"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Euclid Av",
  },
  A44: {
    complex: false,
    draw: true,
    stop_id: "A44",
    stop_name: "Clinton - Washington Avs",
    stop_desc: "",
    stop_lat: "40.683263",
    stop_lon: "-73.965838",
    location_type: "1",
    lines_at: ["C"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Euclid Av",
  },
  A45: {
    complex: false,
    draw: false,
    stop_id: "A45",
    stop_name: "Franklin Av",
    stop_desc: "",
    stop_lat: "40.68138",
    stop_lon: "-73.956848",
    location_type: "1",
    lines_at: ["C"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Euclid Av",
  },
  A46: {
    complex: false,
    draw: true,
    stop_id: "A46",
    stop_name: "Nostrand Av",
    stop_desc: "",
    stop_lat: "40.680438",
    stop_lon: "-73.950426",
    location_type: "1",
    lines_at: ["A", "C"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Lefferts Blvd - Rockaways",
  },
  A47: {
    complex: false,
    draw: true,
    stop_id: "A47",
    stop_name: "Kingston - Throop Avs",
    stop_desc: "",
    stop_lat: "40.679921",
    stop_lon: "-73.940858",
    location_type: "1",
    lines_at: ["C"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Euclid Av",
  },
  A48: {
    complex: false,
    draw: true,
    stop_id: "A48",
    stop_name: "Utica Av",
    stop_desc: "",
    stop_lat: "40.679364",
    stop_lon: "-73.930729",
    location_type: "1",
    lines_at: ["A", "C"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "Lefferts Blvd - Rockaways",
  },
  A49: {
    complex: false,
    draw: true,
    stop_id: "A49",
    stop_name: "Ralph Av",
    stop_desc: "",
    stop_lat: "40.678822",
    stop_lon: "-73.920786",
    location_type: "1",
    lines_at: ["C"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Euclid Av",
  },
  A50: {
    complex: false,
    draw: true,
    stop_id: "A50",
    stop_name: "Rockaway Av",
    stop_desc: "",
    stop_lat: "40.67834",
    stop_lon: "-73.911946",
    location_type: "1",
    lines_at: ["C"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Euclid Av",
  },
  A51: {
    complex: false,
    draw: false,
    stop_id: "A51",
    stop_name: "Broadway Jct",
    stop_desc: "",
    stop_lat: "40.678334",
    stop_lon: "-73.905316",
    location_type: "1",
    lines_at: ["A", "C"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Lefferts Blvd - Rockaways",
  },
  A52: {
    complex: false,
    draw: true,
    stop_id: "A52",
    stop_name: "Liberty Av",
    stop_desc: "",
    stop_lat: "40.674542",
    stop_lon: "-73.896548",
    location_type: "1",
    lines_at: ["C"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Euclid Av",
  },
  A53: {
    complex: false,
    draw: true,
    stop_id: "A53",
    stop_name: "Van Siclen Av",
    stop_desc: "",
    stop_lat: "40.67271",
    stop_lon: "-73.890358",
    location_type: "1",
    lines_at: ["C"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Euclid Av",
  },
  A54: {
    complex: false,
    draw: true,
    stop_id: "A54",
    stop_name: "Shepherd Av",
    stop_desc: "",
    stop_lat: "40.67413",
    stop_lon: "-73.88075",
    location_type: "1",
    lines_at: ["C"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Euclid Av",
  },
  A55: {
    complex: false,
    draw: true,
    stop_id: "A55",
    stop_name: "Euclid Av",
    stop_desc: "",
    stop_lat: "40.675377",
    stop_lon: "-73.872106",
    location_type: "1",
    lines_at: ["A", "C"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "Lefferts - Rockaways",
  },
  A57: {
    complex: false,
    draw: true,
    stop_id: "A57",
    stop_name: "Grant Av",
    stop_desc: "",
    stop_lat: "40.677044",
    stop_lon: "-73.86505",
    location_type: "1",
    lines_at: ["A"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Lefferts - Rockaways",
  },
  A59: {
    complex: false,
    draw: true,
    stop_id: "A59",
    stop_name: "80 St",
    stop_desc: "",
    stop_lat: "40.679371",
    stop_lon: "-73.858992",
    location_type: "1",
    lines_at: ["A"],
    borough: "queens",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Lefferts - Rockaways",
  },
  A60: {
    complex: false,
    draw: true,
    stop_id: "A60",
    stop_name: "88 St",
    stop_desc: "",
    stop_lat: "40.679843",
    stop_lon: "-73.85147",
    location_type: "1",
    lines_at: ["A"],
    borough: "queens",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Lefferts - Rockaways",
  },
  A61: {
    complex: false,
    draw: true,
    stop_id: "A61",
    stop_name: "Rockaway Blvd",
    stop_desc: "",
    stop_lat: "40.680429",
    stop_lon: "-73.843853",
    location_type: "1",
    lines_at: ["A"],
    borough: "queens",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Lefferts - Rockaways",
  },
  A63: {
    complex: false,
    draw: true,
    stop_id: "A63",
    stop_name: "104 St",
    stop_desc: "",
    stop_lat: "40.681711",
    stop_lon: "-73.837683",
    location_type: "1",
    lines_at: ["A"],
    borough: "queens",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Lefferts Blvd",
  },
  A64: {
    complex: false,
    draw: true,
    stop_id: "A64",
    stop_name: "111 St",
    stop_desc: "",
    stop_lat: "40.684331",
    stop_lon: "-73.832163",
    location_type: "1",
    lines_at: ["A"],
    borough: "queens",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Lefferts Blvd",
  },
  A65: {
    complex: false,
    draw: true,
    stop_id: "A65",
    stop_name: "Ozone Park - Lefferts Blvd",
    stop_desc: "",
    stop_lat: "40.685951",
    stop_lon: "-73.825798",
    location_type: "1",
    lines_at: ["A"],
    borough: "queens",
    accessible: true,
    north_label: "Manhattan",
    south_label: "",
  },
  B04: {
    complex: false,
    draw: true,
    stop_id: "B04",
    stop_name: "21 St - Queensbridge",
    stop_desc: "",
    stop_lat: "40.754203",
    stop_lon: "-73.942836",
    location_type: "1",
    lines_at: ["F"],
    borough: "queens",
    accessible: true,
    north_label: "Jamaica",
    south_label: "Manhattan",
  },
  B06: {
    complex: false,
    draw: true,
    stop_id: "B06",
    stop_name: "Roosevelt Island",
    stop_desc: "",
    stop_lat: "40.759145",
    stop_lon: "-73.95326",
    location_type: "1",
    lines_at: ["F"],
    borough: "manhattan",
    accessible: true,
    north_label: "Queens",
    south_label: "Manhattan",
  },
  B08: {
    complex: false,
    draw: true,
    stop_id: "B08",
    stop_name: "Lexington Av/63 St",
    stop_desc: "",
    stop_lat: "40.764629",
    stop_lon: "-73.966113",
    location_type: "1",
    lines_at: ["F", "N", "Q"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & Queens",
    south_label: "Downtown & Brooklyn",
  },
  B10: {
    complex: false,
    draw: true,
    stop_id: "B10",
    stop_name: "57 St",
    stop_desc: "",
    stop_lat: "40.763972",
    stop_lon: "-73.97745",
    location_type: "1",
    lines_at: ["F"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & Queens",
    south_label: "Downtown & Brooklyn",
  },
  B12: {
    complex: false,
    draw: true,
    stop_id: "B12",
    stop_name: "9 Av",
    stop_desc: "",
    stop_lat: "40.646292",
    stop_lon: "-73.994324",
    location_type: "1",
    lines_at: ["D"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  B13: {
    complex: false,
    draw: true,
    stop_id: "B13",
    stop_name: "Fort Hamilton Pkwy",
    stop_desc: "",
    stop_lat: "40.640914",
    stop_lon: "-73.994304",
    location_type: "1",
    lines_at: ["D"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  B14: {
    complex: false,
    draw: true,
    stop_id: "B14",
    stop_name: "50 St",
    stop_desc: "",
    stop_lat: "40.63626",
    stop_lon: "-73.994791",
    location_type: "1",
    lines_at: ["D"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  B15: {
    complex: false,
    draw: true,
    stop_id: "B15",
    stop_name: "55 St",
    stop_desc: "",
    stop_lat: "40.631435",
    stop_lon: "-73.995476",
    location_type: "1",
    lines_at: ["D"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  B16: {
    complex: false,
    draw: false,
    stop_id: "B16",
    stop_name: "62 St",
    stop_desc: "",
    stop_lat: "40.626472",
    stop_lon: "-73.996895",
    location_type: "1",
    lines_at: ["D"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  B17: {
    complex: false,
    draw: true,
    stop_id: "B17",
    stop_name: "71 St",
    stop_desc: "",
    stop_lat: "40.619589",
    stop_lon: "-73.998864",
    location_type: "1",
    lines_at: ["D"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  B18: {
    complex: false,
    draw: true,
    stop_id: "B18",
    stop_name: "79 St",
    stop_desc: "",
    stop_lat: "40.613501",
    stop_lon: "-74.00061",
    location_type: "1",
    lines_at: ["D"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  B19: {
    complex: false,
    draw: true,
    stop_id: "B19",
    stop_name: "18 Av",
    stop_desc: "",
    stop_lat: "40.607954",
    stop_lon: "-74.001736",
    location_type: "1",
    lines_at: ["D"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  B20: {
    complex: false,
    draw: true,
    stop_id: "B20",
    stop_name: "20 Av",
    stop_desc: "",
    stop_lat: "40.604556",
    stop_lon: "-73.998168",
    location_type: "1",
    lines_at: ["D"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  B21: {
    complex: false,
    draw: true,
    stop_id: "B21",
    stop_name: "Bay Pkwy",
    stop_desc: "",
    stop_lat: "40.601875",
    stop_lon: "-73.993728",
    location_type: "1",
    lines_at: ["D"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  B22: {
    complex: false,
    draw: true,
    stop_id: "B22",
    stop_name: "25 Av",
    stop_desc: "",
    stop_lat: "40.597704",
    stop_lon: "-73.986829",
    location_type: "1",
    lines_at: ["D"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  B23: {
    complex: false,
    draw: true,
    stop_id: "B23",
    stop_name: "Bay 50 St",
    stop_desc: "",
    stop_lat: "40.588841",
    stop_lon: "-73.983765",
    location_type: "1",
    lines_at: ["D"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  D01: {
    complex: false,
    draw: true,
    stop_id: "D01",
    stop_name: "Norwood - 205 St",
    stop_desc: "",
    stop_lat: "40.874811",
    stop_lon: "-73.878855",
    location_type: "1",
    lines_at: ["D"],
    borough: "bronx",
    accessible: false,
    north_label: "",
    south_label: "Manhattan",
  },
  D03: {
    complex: false,
    draw: true,
    stop_id: "D03",
    stop_name: "Bedford Park Blvd",
    stop_desc: "",
    stop_lat: "40.873244",
    stop_lon: "-73.887138",
    location_type: "1",
    lines_at: ["B", "D"],
    borough: "bronx",
    accessible: true,
    north_label: "Norwood - 205 St",
    south_label: "Manhattan",
  },
  D04: {
    complex: false,
    draw: true,
    stop_id: "D04",
    stop_name: "Kingsbridge Rd",
    stop_desc: "",
    stop_lat: "40.866978",
    stop_lon: "-73.893509",
    location_type: "1",
    lines_at: ["B", "D"],
    borough: "bronx",
    accessible: true,
    north_label: "Bedford Pk Blvd & 205 St",
    south_label: "Manhattan",
  },
  D05: {
    complex: false,
    draw: true,
    stop_id: "D05",
    stop_name: "Fordham Rd",
    stop_desc: "",
    stop_lat: "40.861296",
    stop_lon: "-73.897749",
    location_type: "1",
    lines_at: ["B", "D"],
    borough: "bronx",
    accessible: false,
    north_label: "Bedford Pk Blvd & 205 St",
    south_label: "Manhattan",
  },
  D06: {
    complex: false,
    draw: true,
    stop_id: "D06",
    stop_name: "182-183 Sts",
    stop_desc: "",
    stop_lat: "40.856093",
    stop_lon: "-73.900741",
    location_type: "1",
    lines_at: ["B", "D"],
    borough: "bronx",
    accessible: false,
    north_label: "Bedford Pk Blvd & 205 St",
    south_label: "Manhattan",
  },
  D07: {
    complex: false,
    draw: true,
    stop_id: "D07",
    stop_name: "Tremont Av",
    stop_desc: "",
    stop_lat: "40.85041",
    stop_lon: "-73.905227",
    location_type: "1",
    lines_at: ["B", "D"],
    borough: "bronx",
    accessible: false,
    north_label: "Bedford Pk Blvd & 205 St",
    south_label: "Manhattan",
  },
  D08: {
    complex: false,
    draw: true,
    stop_id: "D08",
    stop_name: "174-175 Sts",
    stop_desc: "",
    stop_lat: "40.8459",
    stop_lon: "-73.910136",
    location_type: "1",
    lines_at: ["B", "D"],
    borough: "bronx",
    accessible: false,
    north_label: "Bedford Pk Blvd & 205 St",
    south_label: "Manhattan",
  },
  D09: {
    complex: false,
    draw: true,
    stop_id: "D09",
    stop_name: "170 St",
    stop_desc: "",
    stop_lat: "40.839306",
    stop_lon: "-73.9134",
    location_type: "1",
    lines_at: ["B", "D"],
    borough: "bronx",
    accessible: false,
    north_label: "Bedford Pk Blvd & 205 St",
    south_label: "Manhattan",
  },
  D10: {
    complex: false,
    draw: true,
    stop_id: "D10",
    stop_name: "167 St",
    stop_desc: "",
    stop_lat: "40.833771",
    stop_lon: "-73.91844",
    location_type: "1",
    lines_at: ["B", "D"],
    borough: "bronx",
    accessible: false,
    north_label: "Bedford Pk Blvd & 205 St",
    south_label: "Manhattan",
  },
  D11: {
    complex: false,
    draw: false,
    stop_id: "D11",
    stop_name: "161 St - Yankee Stadium",
    stop_desc: "",
    stop_lat: "40.827905",
    stop_lon: "-73.925651",
    location_type: "1",
    lines_at: ["B", "D"],
    borough: "bronx",
    accessible: true,
    north_label: "Bedford Pk Blvd & 205 St",
    south_label: "Manhattan",
  },
  D12: {
    complex: false,
    draw: true,
    stop_id: "D12",
    stop_name: "155 St",
    stop_desc: "",
    stop_lat: "40.830135",
    stop_lon: "-73.938209",
    location_type: "1",
    lines_at: ["B", "D"],
    borough: "bronx",
    accessible: false,
    north_label: "The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  D13: {
    complex: false,
    draw: true,
    stop_id: "D13",
    stop_name: "145 St",
    stop_desc: "",
    stop_lat: "40.824783",
    stop_lon: "-73.944216",
    location_type: "1",
    lines_at: ["A", "B", "C", "D"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Downtown & Brooklyn",
  },
  D14: {
    complex: false,
    draw: true,
    stop_id: "D14",
    stop_name: "7 Av",
    stop_desc: "",
    stop_lat: "40.762862",
    stop_lon: "-73.981637",
    location_type: "1",
    lines_at: ["B", "D", "E"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx - Queens",
    south_label: "Downtown & Brooklyn",
  },
  D15: {
    complex: false,
    draw: true,
    stop_id: "D15",
    stop_name: "47-50 Sts - Rockefeller Ctr",
    stop_desc: "",
    stop_lat: "40.758663",
    stop_lon: "-73.981329",
    location_type: "1",
    lines_at: ["B", "D", "F", "M"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & The Bronx - Queens",
    south_label: "Downtown & Brooklyn",
  },
  D16: {
    complex: false,
    draw: false,
    stop_id: "D16",
    stop_name: "42 St - Bryant Pk",
    stop_desc: "",
    stop_lat: "40.754222",
    stop_lon: "-73.984569",
    location_type: "1",
    lines_at: ["B", "D", "F", "M"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx - Queens",
    south_label: "Downtown & Brooklyn",
  },
  D17: {
    complex: false,
    draw: false,
    stop_id: "D17",
    stop_name: "34 St - Herald Sq",
    stop_desc: "",
    stop_lat: "40.749719",
    stop_lon: "-73.987823",
    location_type: "1",
    lines_at: ["B", "D", "F", "M"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & The Bronx - Queens",
    south_label: "Downtown & Brooklyn",
  },
  D18: {
    complex: false,
    draw: true,
    stop_id: "D18",
    stop_name: "23 St",
    stop_desc: "",
    stop_lat: "40.742878",
    stop_lon: "-73.992821",
    location_type: "1",
    lines_at: ["F", "M",],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & Queens",
    south_label: "Downtown & Brooklyn",
  },
  D19: {
    complex: false,
    draw: false,
    stop_id: "D19",
    stop_name: "14 St",
    stop_desc: "",
    stop_lat: "40.738228",
    stop_lon: "-73.996209",
    location_type: "1",
    lines_at: ["F", "M"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & Queens",
    south_label: "Downtown & Brooklyn",
  },
  D20: {
    complex: false,
    draw: true,
    stop_id: "D20",
    stop_name: "W 4 St - Washington Sq",
    stop_desc: "",
    stop_lat: "40.732338",
    stop_lon: "-74.000495",
    location_type: "1",
    lines_at: ["B", "D", "F", "M"],
    transfers: ["A", "C", "E"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & Queens",
    south_label: "Downtown & Brooklyn",
  },
  D21: {
    complex: false,
    draw: false,
    stop_id: "D21",
    stop_name: "Broadway-Lafayette St",
    stop_desc: "",
    stop_lat: "40.725297",
    stop_lon: "-73.996204",
    location_type: "1",
    lines_at: ["B", "D", "F", "M"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & The Bronx - Queens",
    south_label: "Downtown & Brooklyn",
  },
  D22: {
    complex: false,
    draw: true,
    stop_id: "D22",
    stop_name: "Grand St",
    stop_desc: "",
    stop_lat: "40.718267",
    stop_lon: "-73.993753",
    location_type: "1",
    lines_at: ["B", "D",],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & The Bronx",
    south_label: "Brooklyn",
  },
  D24: {
    complex: false,
    draw: false,
    stop_id: "D24",
    stop_name: "Atlantic Av - Barclays Ctr",
    stop_desc: "",
    stop_lat: "40.68446",
    stop_lon: "-73.97689",
    location_type: "1",
    lines_at: ["B", "D"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "Brighton Beach & Coney Island",
  },
  D25: {
    complex: false,
    draw: true,
    stop_id: "D25",
    stop_name: "7 Av",
    stop_desc: "",
    stop_lat: "40.67705",
    stop_lon: "-73.972367",
    location_type: "1",
    lines_at: ["B", "Q"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Brighton Beach & Coney Island",
  },
  D26: {
    complex: false,
    draw: true,
    stop_id: "D26",
    stop_name: "Prospect Park",
    stop_desc: "",
    stop_lat: "40.661614",
    stop_lon: "-73.962246",
    location_type: "1",
    lines_at: ["B", "Q", "SF"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan & Franklin Av",
    south_label: "Brighton Beach & Coney Island",
  },
  D27: {
    complex: false,
    draw: true,
    stop_id: "D27",
    stop_name: "Parkside Av",
    stop_desc: "",
    stop_lat: "40.655292",
    stop_lon: "-73.961495",
    location_type: "1",
    lines_at: ["Q"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Brighton Beach & Coney Island",
  },
  D28: {
    complex: false,
    draw: true,
    stop_id: "D28",
    stop_name: "Church Av",
    stop_desc: "",
    stop_lat: "40.650527",
    stop_lon: "-73.962982",
    location_type: "1",
    lines_at: ["B", "Q"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Brighton Beach & Coney Island",
  },
  D29: {
    complex: false,
    draw: true,
    stop_id: "D29",
    stop_name: "Beverley Rd",
    stop_desc: "",
    stop_lat: "40.644031",
    stop_lon: "-73.964492",
    location_type: "1",
    lines_at: ["Q"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Brighton Beach & Coney Island",
  },
  D30: {
    complex: false,
    draw: true,
    stop_id: "D30",
    stop_name: "Cortelyou Rd",
    stop_desc: "",
    stop_lat: "40.640927",
    stop_lon: "-73.963891",
    location_type: "1",
    lines_at: ["Q"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Brighton Beach & Coney Island",
  },
  D31: {
    complex: false,
    draw: true,
    stop_id: "D31",
    stop_name: "Newkirk Plaza",
    stop_desc: "",
    stop_lat: "40.635082",
    stop_lon: "-73.962793",
    location_type: "1",
    lines_at: ["B", "Q"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Brighton Beach & Coney Island",
  },
  D32: {
    complex: false,
    draw: true,
    stop_id: "D32",
    stop_name: "Avenue H",
    stop_desc: "",
    stop_lat: "40.62927",
    stop_lon: "-73.961639",
    location_type: "1",
    lines_at: ["Q"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "Brighton Beach & Coney Island",
  },
  D33: {
    complex: false,
    draw: true,
    stop_id: "D33",
    stop_name: "Avenue J",
    stop_desc: "",
    stop_lat: "40.625039",
    stop_lon: "-73.960803",
    location_type: "1",
    lines_at: ["Q"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Brighton Beach & Coney Island",
  },
  D34: {
    complex: false,
    draw: true,
    stop_id: "D34",
    stop_name: "Avenue M",
    stop_desc: "",
    stop_lat: "40.617618",
    stop_lon: "-73.959399",
    location_type: "1",
    lines_at: ["Q"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Brighton Beach & Coney Island",
  },
  D35: {
    complex: false,
    draw: true,
    stop_id: "D35",
    stop_name: "Kings Hwy",
    stop_desc: "",
    stop_lat: "40.60867",
    stop_lon: "-73.957734",
    location_type: "1",
    lines_at: ["B", "Q"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "Brighton Beach & Coney Island",
  },
  D37: {
    complex: false,
    draw: true,
    stop_id: "D37",
    stop_name: "Avenue U",
    stop_desc: "",
    stop_lat: "40.5993",
    stop_lon: "-73.955929",
    location_type: "1",
    lines_at: ["Q"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Brighton Beach & Coney Island",
  },
  D38: {
    complex: false,
    draw: true,
    stop_id: "D38",
    stop_name: "Neck Rd",
    stop_desc: "",
    stop_lat: "40.595246",
    stop_lon: "-73.955161",
    location_type: "1",
    lines_at: ["Q"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Brighton Beach & Coney Island",
  },
  D39: {
    complex: false,
    draw: true,
    stop_id: "D39",
    stop_name: "Sheepshead Bay",
    stop_desc: "",
    stop_lat: "40.586896",
    stop_lon: "-73.954155",
    location_type: "1",
    lines_at: ["B", "Q"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Brighton Beach & Coney Island",
  },
  D40: {
    complex: false,
    draw: true,
    stop_id: "D40",
    stop_name: "Brighton Beach",
    stop_desc: "",
    stop_lat: "40.577621",
    stop_lon: "-73.961376",
    location_type: "1",
    lines_at: ["B", "Q"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  D41: {
    complex: false,
    draw: true,
    stop_id: "D41",
    stop_name: "Ocean Pkwy",
    stop_desc: "",
    stop_lat: "40.576312",
    stop_lon: "-73.968501",
    location_type: "1",
    lines_at: ["Q"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Stillwell Av",
  },
  D42: {
    complex: false,
    draw: true,
    stop_id: "D42",
    stop_name: "W 8 St - NY Aquarium",
    stop_desc: "",
    stop_lat: "40.576127",
    stop_lon: "-73.975939",
    location_type: "1",
    lines_at: ["F", "Q"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Stillwell Av",
  },
  D43: {
    complex: false,
    draw: true,
    stop_id: "D43",
    stop_name: "Coney Island - Stillwell Av",
    stop_desc: "",
    stop_lat: "40.577422",
    stop_lon: "-73.981233",
    location_type: "1",
    lines_at: ["D", "F", "N", "Q"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "",
  },
  E01: {
    complex: false,
    draw: false,
    stop_id: "E01",
    stop_name: "World Trade Center",
    stop_desc: "",
    stop_lat: "40.712582",
    stop_lon: "-74.009781",
    location_type: "1",
    lines_at: ["E"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & Queens",
    south_label: "",
  },
  F01: {
    complex: false,
    draw: true,
    stop_id: "F01",
    stop_name: "Jamaica - 179 St",
    stop_desc: "",
    stop_lat: "40.712646",
    stop_lon: "-73.783817",
    location_type: "1",
    lines_at: ["E", "F"],
    borough: "queens",
    accessible: true,
    north_label: "",
    south_label: "Manhattan",
  },
  F02: {
    complex: false,
    draw: true,
    stop_id: "F02",
    stop_name: "169 St",
    stop_desc: "",
    stop_lat: "40.71047",
    stop_lon: "-73.793604",
    location_type: "1",
    lines_at: ["E", "F"],
    borough: "queens",
    accessible: false,
    north_label: "179 St",
    south_label: "Manhattan",
  },
  F03: {
    complex: false,
    draw: true,
    stop_id: "F03",
    stop_name: "Parsons Blvd",
    stop_desc: "",
    stop_lat: "40.707564",
    stop_lon: "-73.803326",
    location_type: "1",
    lines_at: ["E", "F"],
    borough: "queens",
    accessible: false,
    north_label: "179 St",
    south_label: "Manhattan",
  },
  F04: {
    complex: false,
    draw: true,
    stop_id: "F04",
    stop_name: "Sutphin Blvd",
    stop_desc: "",
    stop_lat: "40.70546",
    stop_lon: "-73.810708",
    location_type: "1",
    lines_at: ["E", "F"],
    borough: "queens",
    accessible: false,
    north_label: "179 St",
    south_label: "Manhattan",
  },
  F05: {
    complex: false,
    draw: true,
    stop_id: "F05",
    stop_name: "Briarwood - Van Wyck Blvd",
    stop_desc: "",
    stop_lat: "40.709179",
    stop_lon: "-73.820574",
    location_type: "1",
    lines_at: ["E", "F"],
    borough: "queens",
    accessible: false,
    north_label: "Jamaica",
    south_label: "Manhattan",
  },
  F06: {
    complex: false,
    draw: true,
    stop_id: "F06",
    stop_name: "Kew Gardens - Union Tpke",
    stop_desc: "",
    stop_lat: "40.714441",
    stop_lon: "-73.831008",
    location_type: "1",
    lines_at: ["E", "F"],
    borough: "queens",
    accessible: true,
    north_label: "Jamaica",
    south_label: "Manhattan",
  },
  F07: {
    complex: false,
    draw: true,
    stop_id: "F07",
    stop_name: "75 Av",
    stop_desc: "",
    stop_lat: "40.718331",
    stop_lon: "-73.837324",
    location_type: "1",
    lines_at: ["E", "F"],
    borough: "queens",
    accessible: false,
    north_label: "Jamaica",
    south_label: "Manhattan",
  },
  F09: {
    complex: false,
    draw: false,
    stop_id: "F09",
    stop_name: "Court Sq - 23 St",
    stop_desc: "",
    stop_lat: "40.747846",
    stop_lon: "-73.946",
    location_type: "1",
    lines_at: ["E", "M"],
    transfers: ["7", "G"],
    borough: "queens",
    accessible: true,
    north_label: "Forest Hills - Jamaica",
    south_label: "Manhattan",
  },
  F11: {
    complex: false,
    draw: false,
    stop_id: "F11",
    stop_name: "Lexington Av/53 St",
    stop_desc: "",
    stop_lat: "40.757552",
    stop_lon: "-73.969055",
    location_type: "1",
    lines_at: ["E", "M"],
    borough: "manhattan",
    accessible: true,
    north_label: "Queens",
    south_label: "Downtown & Brooklyn",
  },
  F12: {
    complex: false,
    draw: true,
    stop_id: "F12",
    stop_name: "5 Av/53 St",
    stop_desc: "",
    stop_lat: "40.760167",
    stop_lon: "-73.975224",
    location_type: "1",
    lines_at: ["E", "M"],
    borough: "manhattan",
    accessible: false,
    north_label: "Queens",
    south_label: "Downtown & Brooklyn",
  },
  F14: {
    complex: false,
    draw: true,
    stop_id: "F14",
    stop_name: "2 Av",
    stop_desc: "",
    stop_lat: "40.723402",
    stop_lon: "-73.989938",
    location_type: "1",
    lines_at: ["F"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & Queens",
    south_label: "Downtown & Brooklyn",
  },
  F15: {
    complex: false,
    draw: false,
    stop_id: "F15",
    stop_name: "Delancey St",
    stop_desc: "Delancey St-Essex St",
    stop_lat: "40.718611",
    stop_lon: "-73.988114",
    location_type: "1",
    lines_at: ["F", "M"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & Queens",
    south_label: "Downtown & Brooklyn",
  },
  F16: {
    complex: false,
    draw: true,
    stop_id: "F16",
    stop_name: "East Broadway",
    stop_desc: "",
    stop_lat: "40.713715",
    stop_lon: "-73.990173",
    location_type: "1",
    lines_at: ["F"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & Queens",
    south_label: "Brooklyn",
  },
  F18: {
    complex: false,
    draw: true,
    stop_id: "F18",
    stop_name: "York St",
    stop_desc: "",
    stop_lat: "40.701397",
    stop_lon: "-73.986751",
    location_type: "1",
    lines_at: ["F"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  F20: {
    complex: false,
    draw: true,
    stop_id: "F20",
    stop_name: "Bergen St",
    stop_desc: "",
    stop_lat: "40.686145",
    stop_lon: "-73.990862",
    location_type: "1",
    lines_at: ["F", "G"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan - Queens",
    south_label: "Church Ave - Coney Island",
  },
  F21: {
    complex: false,
    draw: true,
    stop_id: "F21",
    stop_name: "Carroll St",
    stop_desc: "",
    stop_lat: "40.680303",
    stop_lon: "-73.995048",
    location_type: "1",
    lines_at: ["F", "G"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan - Queens",
    south_label: "Church Ave - Coney Island",
  },
  F22: {
    complex: false,
    draw: true,
    stop_id: "F22",
    stop_name: "Smith - 9 Sts",
    stop_desc: "",
    stop_lat: "40.67358",
    stop_lon: "-73.995959",
    location_type: "1",
    lines_at: ["F", "G"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan - Queens",
    south_label: "Church Ave - Coney Island",
  },
  F23: {
    complex: false,
    draw: false,
    stop_id: "F23",
    stop_name: "4 Av",
    stop_desc: "4 Av - 9 St",
    stop_lat: "40.670272",
    stop_lon: "-73.989779",
    location_type: "1",
    lines_at: ["F", "G"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan - Queens",
    south_label: "Church Ave - Coney Island",
  },
  F24: {
    complex: false,
    draw: true,
    stop_id: "F24",
    stop_name: "7 Av",
    stop_desc: "",
    stop_lat: "40.666271",
    stop_lon: "-73.980305",
    location_type: "1",
    lines_at: ["F", "G"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan - Queens",
    south_label: "Church Ave - Coney Island",
  },
  F25: {
    complex: false,
    draw: true,
    stop_id: "F25",
    stop_name: "15 St - Prospect Park",
    stop_desc: "",
    stop_lat: "40.660365",
    stop_lon: "-73.979493",
    location_type: "1",
    lines_at: ["F", "G"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan - Queens",
    south_label: "Church Ave - Coney Island",
  },
  F26: {
    complex: false,
    draw: true,
    stop_id: "F26",
    stop_name: "Fort Hamilton Pkwy",
    stop_desc: "",
    stop_lat: "40.650782",
    stop_lon: "-73.975776",
    location_type: "1",
    lines_at: ["F", "G"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan - Queens",
    south_label: "Church Ave - Coney Island",
  },
  F27: {
    complex: false,
    draw: true,
    stop_id: "F27",
    stop_name: "Church Av",
    stop_desc: "",
    stop_lat: "40.644041",
    stop_lon: "-73.979678",
    location_type: "1",
    lines_at: ["F", "G"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  F29: {
    complex: false,
    draw: true,
    stop_id: "F29",
    stop_name: "Ditmas Av",
    stop_desc: "",
    stop_lat: "40.636119",
    stop_lon: "-73.978172",
    location_type: "1",
    lines_at: ["F"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  F30: {
    complex: false,
    draw: true,
    stop_id: "F30",
    stop_name: "18 Av",
    stop_desc: "",
    stop_lat: "40.629755",
    stop_lon: "-73.976971",
    location_type: "1",
    lines_at: ["F"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  F31: {
    complex: false,
    draw: true,
    stop_id: "F31",
    stop_name: "Avenue I",
    stop_desc: "",
    stop_lat: "40.625322",
    stop_lon: "-73.976127",
    location_type: "1",
    lines_at: ["F"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  F32: {
    complex: false,
    draw: true,
    stop_id: "F32",
    stop_name: "Bay Pkwy",
    stop_desc: "",
    stop_lat: "40.620769",
    stop_lon: "-73.975264",
    location_type: "1",
    lines_at: ["F"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  F33: {
    complex: false,
    draw: true,
    stop_id: "F33",
    stop_name: "Avenue N",
    stop_desc: "",
    stop_lat: "40.61514",
    stop_lon: "-73.974197",
    location_type: "1",
    lines_at: ["F"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  F34: {
    complex: false,
    draw: true,
    stop_id: "F34",
    stop_name: "Avenue P",
    stop_desc: "",
    stop_lat: "40.608944",
    stop_lon: "-73.973022",
    location_type: "1",
    lines_at: ["F"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  F35: {
    complex: false,
    draw: true,
    stop_id: "F35",
    stop_name: "Kings Hwy",
    stop_desc: "",
    stop_lat: "40.603217",
    stop_lon: "-73.972361",
    location_type: "1",
    lines_at: ["F"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  F36: {
    complex: false,
    draw: true,
    stop_id: "F36",
    stop_name: "Avenue U",
    stop_desc: "",
    stop_lat: "40.596063",
    stop_lon: "-73.973357",
    location_type: "1",
    lines_at: ["F"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  F38: {
    complex: false,
    draw: true,
    stop_id: "F38",
    stop_name: "Avenue X",
    stop_desc: "",
    stop_lat: "40.58962",
    stop_lon: "-73.97425",
    location_type: "1",
    lines_at: ["F"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  F39: {
    complex: false,
    draw: true,
    stop_id: "F39",
    stop_name: "Neptune Av",
    stop_desc: "",
    stop_lat: "40.581011",
    stop_lon: "-73.974574",
    location_type: "1",
    lines_at: ["F"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Stillwell Av",
  },
  G05: {
    complex: false,
    draw: true,
    stop_id: "G05",
    stop_name: "Jamaica Center - Parsons/Archer",
    stop_desc: "",
    stop_lat: "40.702147",
    stop_lon: "-73.801109",
    location_type: "1",
    lines_at: ["E", "J", "Z"],
    borough: "queens",
    accessible: true,
    north_label: "",
    south_label: "Manhattan",
  },
  G06: {
    complex: false,
    draw: true,
    stop_id: "G06",
    stop_name: "Sutphin Blvd - Archer Av - JFK Airport",
    stop_desc: "",
    stop_lat: "40.700486",
    stop_lon: "-73.807969",
    location_type: "1",
    lines_at: ["E", "J", "Z"],
    borough: "queens",
    accessible: true,
    north_label: "Jamaica Center",
    south_label: "Manhattan",
  },
  G07: {
    complex: false,
    draw: true,
    stop_id: "G07",
    stop_name: "Jamaica - Van Wyck",
    stop_desc: "",
    stop_lat: "40.702566",
    stop_lon: "-73.816859",
    location_type: "1",
    lines_at: ["E"],
    borough: "queens",
    accessible: true,
    north_label: "Jamaica Center",
    south_label: "Manhattan",
  },
  G08: {
    complex: false,
    draw: true,
    stop_id: "G08",
    stop_name: "Forest Hills - 71 Av",
    stop_desc: "",
    stop_lat: "40.721691",
    stop_lon: "-73.844521",
    location_type: "1",
    lines_at: ["E", "F", "M", "R"],
    borough: "queens",
    accessible: true,
    north_label: "Jamaica",
    south_label: "Manhattan",
  },
  G09: {
    complex: false,
    draw: true,
    stop_id: "G09",
    stop_name: "67 Av",
    stop_desc: "",
    stop_lat: "40.726523",
    stop_lon: "-73.852719",
    location_type: "1",
    lines_at: ["M", "R"],
    borough: "queens",
    accessible: false,
    north_label: "Forest Hills",
    south_label: "Manhattan",
  },
  G10: {
    complex: false,
    draw: true,
    stop_id: "G10",
    stop_name: "63 Dr - Rego Park",
    stop_desc: "",
    stop_lat: "40.729846",
    stop_lon: "-73.861604",
    location_type: "1",
    lines_at: ["M", "R"],
    borough: "queens",
    accessible: false,
    north_label: "Forest Hills",
    south_label: "Manhattan",
  },
  G11: {
    complex: false,
    draw: true,
    stop_id: "G11",
    stop_name: "Woodhaven Blvd",
    stop_desc: "",
    stop_lat: "40.733106",
    stop_lon: "-73.869229",
    location_type: "1",
    lines_at: ["M", "R"],
    borough: "queens",
    accessible: false,
    north_label: "Forest Hills",
    south_label: "Manhattan",
  },
  G12: {
    complex: false,
    draw: true,
    stop_id: "G12",
    stop_name: "Grand Av - Newtown",
    stop_desc: "",
    stop_lat: "40.737015",
    stop_lon: "-73.877223",
    location_type: "1",
    lines_at: ["M", "R"],
    borough: "queens",
    accessible: false,
    north_label: "Forest Hills",
    south_label: "Manhattan",
  },
  G13: {
    complex: false,
    draw: true,
    stop_id: "G13",
    stop_name: "Elmhurst Av",
    stop_desc: "",
    stop_lat: "40.742454",
    stop_lon: "-73.882017",
    location_type: "1",
    lines_at: ["M", "R"],
    borough: "queens",
    accessible: false,
    north_label: "Forest Hills",
    south_label: "Manhattan",
  },
  G14: {
    complex: false,
    draw: false,
    stop_id: "G14",
    stop_name: "Jackson Hts - Roosevelt Av",
    stop_desc: "",
    stop_lat: "40.746644",
    stop_lon: "-73.891338",
    location_type: "1",
    lines_at: ["E", "F", "M", "R"],
    borough: "queens",
    accessible: true,
    north_label: "Forest Hills - Jamaica",
    south_label: "Manhattan",
  },
  G15: {
    complex: false,
    draw: true,
    stop_id: "G15",
    stop_name: "65 St",
    stop_desc: "",
    stop_lat: "40.749669",
    stop_lon: "-73.898453",
    location_type: "1",
    lines_at: ["M", "R"],
    borough: "queens",
    accessible: false,
    north_label: "Forest Hills",
    south_label: "Manhattan",
  },
  G16: {
    complex: false,
    draw: true,
    stop_id: "G16",
    stop_name: "Northern Blvd",
    stop_desc: "",
    stop_lat: "40.752885",
    stop_lon: "-73.906006",
    location_type: "1",
    lines_at: ["M", "R"],
    borough: "queens",
    accessible: false,
    north_label: "Forest Hills",
    south_label: "Manhattan",
  },
  G18: {
    complex: false,
    draw: true,
    stop_id: "G18",
    stop_name: "46 St",
    stop_desc: "",
    stop_lat: "40.756312",
    stop_lon: "-73.913333",
    location_type: "1",
    lines_at: ["M", "R"],
    borough: "queens",
    accessible: false,
    north_label: "Forest Hills",
    south_label: "Manhattan",
  },
  G19: {
    complex: false,
    draw: true,
    stop_id: "G19",
    stop_name: "Steinway St",
    stop_desc: "",
    stop_lat: "40.756879",
    stop_lon: "-73.92074",
    location_type: "1",
    lines_at: ["M", "R"],
    borough: "queens",
    accessible: false,
    north_label: "Forest Hills",
    south_label: "Manhattan",
  },
  G20: {
    complex: false,
    draw: true,
    stop_id: "G20",
    stop_name: "36 St",
    stop_desc: "",
    stop_lat: "40.752039",
    stop_lon: "-73.928781",
    location_type: "1",
    lines_at: ["M", "R"],
    borough: "queens",
    accessible: false,
    north_label: "Forest Hills",
    south_label: "Manhattan",
  },
  G21: {
    complex: false,
    draw: true,
    stop_id: "G21",
    stop_name: "Queens Plaza",
    stop_desc: "",
    stop_lat: "40.748973",
    stop_lon: "-73.937243",
    location_type: "1",
    lines_at: ["E", "M", "R"],
    borough: "queens",
    accessible: true,
    north_label: "Forest Hills - Jamaica",
    south_label: "Manhattan",
  },
  G22: {
    complex: false,
    draw: false,
    stop_id: "G22",
    stop_name: "Court Sq",
    stop_desc: "",
    stop_lat: "40.746554",
    stop_lon: "-73.943832",
    location_type: "1",
    lines_at: ["G"],
    transfers: ["7", "E", "M"],
    borough: "queens",
    accessible: true,
    north_label: "",
    south_label: "Brooklyn",
  },
  G24: {
    complex: false,
    draw: true,
    stop_id: "G24",
    stop_name: "21 St",
    stop_desc: "",
    stop_lat: "40.744065",
    stop_lon: "-73.949724",
    location_type: "1",
    lines_at: ["G"],
    borough: "queens",
    accessible: false,
    north_label: "Court Sq",
    south_label: "Brooklyn",
  },
  G26: {
    complex: false,
    draw: true,
    stop_id: "G26",
    stop_name: "Greenpoint Av",
    stop_desc: "",
    stop_lat: "40.731352",
    stop_lon: "-73.954449",
    location_type: "1",
    lines_at: ["G"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Queens",
    south_label: "Church Av",
  },
  G28: {
    complex: false,
    draw: true,
    stop_id: "G28",
    stop_name: "Nassau Av",
    stop_desc: "",
    stop_lat: "40.724635",
    stop_lon: "-73.951277",
    location_type: "1",
    lines_at: ["G"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Queens",
    south_label: "Church Av",
  },
  G29: {
    complex: false,
    draw: false,
    stop_id: "G29",
    stop_name: "Metropolitan Av",
    stop_desc: "",
    stop_lat: "40.712792",
    stop_lon: "-73.951418",
    location_type: "1",
    lines_at: ["G"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Queens",
    south_label: "Church Av",
  },
  G30: {
    complex: false,
    draw: true,
    stop_id: "G30",
    stop_name: "Broadway",
    stop_desc: "",
    stop_lat: "40.706092",
    stop_lon: "-73.950308",
    location_type: "1",
    lines_at: ["G"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Queens",
    south_label: "Church Av",
  },
  G31: {
    complex: false,
    draw: true,
    stop_id: "G31",
    stop_name: "Flushing Av",
    stop_desc: "",
    stop_lat: "40.700377",
    stop_lon: "-73.950234",
    location_type: "1",
    lines_at: ["G"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Queens",
    south_label: "Church Av",
  },
  G32: {
    complex: false,
    draw: true,
    stop_id: "G32",
    stop_name: "Myrtle - Willoughby Avs",
    stop_desc: "",
    stop_lat: "40.694568",
    stop_lon: "-73.949046",
    location_type: "1",
    lines_at: ["G"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Queens",
    south_label: "Church Av",
  },
  G33: {
    complex: false,
    draw: true,
    stop_id: "G33",
    stop_name: "Bedford - Nostrand Avs",
    stop_desc: "",
    stop_lat: "40.689627",
    stop_lon: "-73.953522",
    location_type: "1",
    lines_at: ["G"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Queens",
    south_label: "Church Av",
  },
  G34: {
    complex: false,
    draw: true,
    stop_id: "G34",
    stop_name: "Classon Av",
    stop_desc: "",
    stop_lat: "40.688873",
    stop_lon: "-73.96007",
    location_type: "1",
    lines_at: ["G"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Queens",
    south_label: "Church Av",
  },
  G35: {
    complex: false,
    draw: true,
    stop_id: "G35",
    stop_name: "Clinton - Washington Avs",
    stop_desc: "",
    stop_lat: "40.688089",
    stop_lon: "-73.966839",
    location_type: "1",
    lines_at: ["G"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Queens",
    south_label: "Church Av",
  },
  G36: {
    complex: false,
    draw: true,
    stop_id: "G36",
    stop_name: "Fulton St",
    stop_desc: "",
    stop_lat: "40.687119",
    stop_lon: "-73.975375",
    location_type: "1",
    lines_at: ["G"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Queens",
    south_label: "Church Av",
  },
  H01: {
    complex: false, // H is the A line extention to Rockaways
    stop_id: "H01",
    stop_name: "Aqueduct Racetrack",
    stop_desc: "",
    stop_lat: "40.668234",
    stop_lon: "-73.834058",
    location_type: "1",
    lines_at: ["A"],
    borough: "queens",
    accessible: true,
    north_label: "Manhattan",
    south_label: "Rockaways",
  },
  H02: {
    complex: false,
    draw: true,
    stop_id: "H02",
    stop_name: "Aqueduct - N Conduit Av",
    stop_desc: "",
    stop_lat: "40.668234",
    stop_lon: "-73.834058",
    location_type: "1",
    lines_at: ["A"],
    borough: "queens",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Rockaways",
  },
  H03: {
    complex: false,
    draw: true,
    stop_id: "H03",
    stop_name: "Howard Beach - JFK Airport",
    stop_desc: "",
    stop_lat: "40.660476",
    stop_lon: "-73.830301",
    location_type: "1",
    lines_at: ["A"],
    borough: "queens",
    accessible: true,
    north_label: "Manhattan",
    south_label: "Rockaways",
  },
  H04: {
    complex: false,
    draw: true,
    stop_id: "H04",
    stop_name: "Broad Channel",
    stop_desc: "",
    stop_lat: "40.608382",
    stop_lon: "-73.815925",
    location_type: "1",
    lines_at: ["A", "SR"],
    borough: "queens",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Rockaways",
  },
  H06: {
    complex: false,
    draw: true,
    stop_id: "H06",
    stop_name: "Beach 67 St",
    stop_desc: "",
    stop_lat: "40.590927",
    stop_lon: "-73.796924",
    location_type: "1",
    lines_at: ["A"],
    borough: "queens",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Far Rockaway",
  },
  H07: {
    complex: false,
    draw: true,
    stop_id: "H07",
    stop_name: "Beach 60 St",
    stop_desc: "",
    stop_lat: "40.592374",
    stop_lon: "-73.788522",
    location_type: "1",
    lines_at: ["A"],
    borough: "queens",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Far Rockaway",
  },
  H08: {
    complex: false,
    draw: true,
    stop_id: "H08",
    stop_name: "Beach 44 St",
    stop_desc: "",
    stop_lat: "40.592943",
    stop_lon: "-73.776013",
    location_type: "1",
    lines_at: ["A"],
    borough: "queens",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Far Rockaway",
  },
  H09: {
    complex: false,
    draw: true,
    stop_id: "H09",
    stop_name: "Beach 36 St",
    stop_desc: "",
    stop_lat: "40.595398",
    stop_lon: "-73.768175",
    location_type: "1",
    lines_at: ["A"],
    borough: "queens",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Far Rockaway",
  },
  H10: {
    complex: false,
    draw: true,
    stop_id: "H10",
    stop_name: "Beach 25 St",
    stop_desc: "",
    stop_lat: "40.600066",
    stop_lon: "-73.761353",
    location_type: "1",
    lines_at: ["A"],
    borough: "queens",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Far Rockaway",
  },
  H11: {
    complex: false,
    draw: true,
    stop_id: "H11",
    stop_name: "Far Rockaway - Mott Av",
    stop_desc: "",
    stop_lat: "40.603995",
    stop_lon: "-73.755405",
    location_type: "1",
    lines_at: ["A"],
    borough: "queens",
    accessible: true,
    north_label: "Manhattan",
    south_label: "",
  },
  H12: {
    complex: false,
    draw: true,
    stop_id: "H12",
    stop_name: "Beach 90 St",
    stop_desc: "",
    stop_lat: "40.588034",
    stop_lon: "-73.813641",
    location_type: "1",
    lines_at: ["A", "SR"],
    borough: "queens",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Rockaway Park",
  },
  H13: {
    complex: false,
    draw: true,
    stop_id: "H13",
    stop_name: "Beach 98 St",
    stop_desc: "",
    stop_lat: "40.585307",
    stop_lon: "-73.820558",
    location_type: "1",
    lines_at: ["A", "SR"],
    borough: "queens",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Rockaway Park",
  },
  H14: {
    complex: false,
    draw: true,
    stop_id: "H14",
    stop_name: "Beach 105 St",
    stop_desc: "",
    stop_lat: "40.583209",
    stop_lon: "-73.827559",
    location_type: "1",
    lines_at: ["A", "SR"],
    borough: "queens",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Rockaway Park",
  },
  H15: {
    complex: false,
    draw: true,
    stop_id: "H15",
    stop_name: "Rockaway Park - Beach 116 St",
    stop_desc: "",
    stop_lat: "40.580903",
    stop_lon: "-73.835592",
    location_type: "1",
    lines_at: ["A", "SR"],
    borough: "queens",
    accessible: true,
    north_label: "Manhattan",
    south_label: "",
  },
  // H19: { // why another broad channel?
  //   complex: false,
    draw: true,
  //   stop_id: "H19",
  //   stop_name: "Broad Channel",
  //   stop_desc: "",
  //   stop_lat: "40.609014",
  //   stop_lon: "-73.816024",
  //   location_type: "1",
  //   lines_at: ["A", "SR"],
  //   borough: "queens",
  //   accessible: false,
  //   north_label: "Manhattan",
  //   south_label: "Rockaways"
  // },
  J12: {
    complex: false,
    draw: true,
    stop_id: "J12",
    stop_name: "121 St",
    stop_desc: "",
    stop_lat: "40.700492",
    stop_lon: "-73.828294",
    location_type: "1",
    lines_at: ["J", "Z"],
    borough: "queens",
    accessible: false,
    north_label: "Jamaica",
    south_label: "Manhattan",
  },
  J13: {
    complex: false,
    draw: true,
    stop_id: "J13",
    stop_name: "111 St",
    stop_desc: "",
    stop_lat: "40.697418",
    stop_lon: "-73.836345",
    location_type: "1",
    lines_at: ["J"],
    borough: "queens",
    accessible: false,
    north_label: "Jamaica",
    south_label: "Manhattan",
  },
  J14: {
    complex: false,
    draw: true,
    stop_id: "J14",
    stop_name: "104 St",
    stop_desc: "",
    stop_lat: "40.695178",
    stop_lon: "-73.84433",
    location_type: "1",
    lines_at: ["J", "Z"],
    borough: "queens",
    accessible: false,
    north_label: "Jamaica",
    south_label: "Manhattan",
  },
  J15: {
    complex: false,
    draw: true,
    stop_id: "J15",
    stop_name: "Woodhaven Blvd",
    stop_desc: "",
    stop_lat: "40.693879",
    stop_lon: "-73.851576",
    location_type: "1",
    lines_at: ["J", "Z"],
    borough: "queens",
    accessible: false,
    north_label: "Jamaica",
    south_label: "Manhattan",
  },
  J16: {
    complex: false,
    draw: true,
    stop_id: "J16",
    stop_name: "85 St - Forest Pkwy",
    stop_desc: "",
    stop_lat: "40.692435",
    stop_lon: "-73.86001",
    location_type: "1",
    lines_at: ["J"],
    borough: "queens",
    accessible: false,
    north_label: "Jamaica",
    south_label: "Manhattan",
  },
  J17: {
    complex: false,
    draw: true,
    stop_id: "J17",
    stop_name: "75 St",
    stop_desc: "",
    stop_lat: "40.691324",
    stop_lon: "-73.867139",
    location_type: "1",
    lines_at: ["J", "Z"],
    borough: "queens",
    accessible: false,
    north_label: "Jamaica",
    south_label: "Manhattan",
  },
  J19: {
    complex: false,
    draw: true,
    stop_id: "J19",
    stop_name: "Cypress Hills",
    stop_desc: "",
    stop_lat: "40.689941",
    stop_lon: "-73.87255",
    location_type: "1",
    lines_at: ["J"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Jamaica",
    south_label: "Manhattan",
  },
  J20: {
    complex: false,
    draw: true,
    stop_id: "J20",
    stop_name: "Crescent St",
    stop_desc: "",
    stop_lat: "40.683194",
    stop_lon: "-73.873785",
    location_type: "1",
    lines_at: ["J", "Z"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Jamaica",
    south_label: "Manhattan",
  },
  J21: {
    complex: false,
    draw: true,
    stop_id: "J21",
    stop_name: "Norwood Av",
    stop_desc: "",
    stop_lat: "40.68141",
    stop_lon: "-73.880039",
    location_type: "1",
    lines_at: ["J", "Z"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Jamaica",
    south_label: "Manhattan",
  },
  J22: {
    complex: false,
    draw: true,
    stop_id: "J22",
    stop_name: "Cleveland St",
    stop_desc: "",
    stop_lat: "40.679947",
    stop_lon: "-73.884639",
    location_type: "1",
    lines_at: ["J"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Jamaica",
    south_label: "Manhattan",
  },
  J23: {
    complex: false,
    draw: true,
    stop_id: "J23",
    stop_name: "Van Siclen Av",
    stop_desc: "",
    stop_lat: "40.678024",
    stop_lon: "-73.891688",
    location_type: "1",
    lines_at: ["J", "Z"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Jamaica",
    south_label: "Manhattan",
  },
  J24: {
    complex: false,
    draw: true,
    stop_id: "J24",
    stop_name: "Alabama Av",
    stop_desc: "",
    stop_lat: "40.676992",
    stop_lon: "-73.898654",
    location_type: "1",
    lines_at: ["J", "Z"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Jamaica",
    south_label: "Manhattan",
  },
  J27: {
    complex: false,
    draw: false,
    stop_id: "J27",
    stop_name: "Broadway Jct",
    stop_desc: "",
    stop_lat: "40.679498",
    stop_lon: "-73.904512",
    location_type: "1",
    lines_at: ["J", "Z"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Jamaica",
    south_label: "Manhattan",
  },
  J28: {
    complex: false,
    draw: true,
    stop_id: "J28",
    stop_name: "Chauncey St",
    stop_desc: "",
    stop_lat: "40.682893",
    stop_lon: "-73.910456",
    location_type: "1",
    lines_at: ["J", "Z"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Jamaica",
    south_label: "Manhattan",
  },
  J29: {
    complex: false,
    draw: true,
    stop_id: "J29",
    stop_name: "Halsey St",
    stop_desc: "",
    stop_lat: "40.68637",
    stop_lon: "-73.916559",
    location_type: "1",
    lines_at: ["J"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Jamaica",
    south_label: "Manhattan",
  },
  J30: {
    complex: false,
    draw: true,
    stop_id: "J30",
    stop_name: "Gates Av",
    stop_desc: "",
    stop_lat: "40.68963",
    stop_lon: "-73.92227",
    location_type: "1",
    lines_at: ["J", "Z"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Jamaica",
    south_label: "Manhattan",
  },
  J31: {
    complex: false,
    draw: true,
    stop_id: "J31",
    stop_name: "Kosciuszko St",
    stop_desc: "",
    stop_lat: "40.693342",
    stop_lon: "-73.928814",
    location_type: "1",
    lines_at: ["J"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Jamaica",
    south_label: "Manhattan",
  },
  L01: {
    complex: false,
    draw: false,
    stop_id: "L01",
    stop_name: "8 Av",
    stop_desc: "",
    stop_lat: "40.739777",
    stop_lon: "-74.002578",
    location_type: "1",
    lines_at: ["L"],
    borough: "manhattan",
    accessible: true,
    north_label: "8 Av",
    south_label: "Brooklyn",
  },
  L02: {
    complex: false,
    draw: false,
    stop_id: "L02",
    stop_name: "6 Av",
    stop_desc: "",
    stop_lat: "40.737335",
    stop_lon: "-73.996786",
    location_type: "1",
    lines_at: ["L"],
    borough: "manhattan",
    accessible: false,
    north_label: "8 Av",
    south_label: "Brooklyn",
  },
  L03: {
    complex: false,
    draw: false,
    stop_id: "L03",
    stop_name: "Union Sq - 14 St",
    stop_desc: "",
    stop_lat: "40.734789",
    stop_lon: "-73.99073",
    location_type: "1",
    lines_at: ["L"],
    borough: "manhattan",
    accessible: true,
    north_label: "8 Av",
    south_label: "Brooklyn",
  },
  L05: {
    complex: false,
    draw: true,
    stop_id: "L05",
    stop_name: "3 Av",
    stop_desc: "",
    stop_lat: "40.732849",
    stop_lon: "-73.986122",
    location_type: "1",
    lines_at: ["L"],
    borough: "manhattan",
    accessible: false,
    north_label: "8 Av",
    south_label: "Brooklyn",
  },
  L06: {
    complex: false,
    draw: true,
    stop_id: "L06",
    stop_name: "1 Av",
    stop_desc: "",
    stop_lat: "40.730953",
    stop_lon: "-73.981628",
    location_type: "1",
    lines_at: ["L"],
    borough: "manhattan",
    accessible: true,
    north_label: "8 Av",
    south_label: "Brooklyn",
  },
  L08: {
    complex: false,
    draw: true,
    stop_id: "L08",
    stop_name: "Bedford Av",
    stop_desc: "",
    stop_lat: "40.717304",
    stop_lon: "-73.956872",
    location_type: "1",
    lines_at: ["L"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "Canarsie - Rockaway Pkwy",
  },
  L10: {
    complex: false,
    draw: false,
    stop_id: "L10",
    stop_name: "Lorimer St",
    stop_desc: "",
    stop_lat: "40.714063",
    stop_lon: "-73.950275",
    location_type: "1",
    lines_at: ["L"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Canarsie - Rockaway Pkwy",
  },
  L11: {
    complex: false,
    draw: true,
    stop_id: "L11",
    stop_name: "Graham Av",
    stop_desc: "",
    stop_lat: "40.714565",
    stop_lon: "-73.944053",
    location_type: "1",
    lines_at: ["L"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Canarsie - Rockaway Pkwy",
  },
  L12: {
    complex: false,
    draw: true,
    stop_id: "L12",
    stop_name: "Grand St",
    stop_desc: "",
    stop_lat: "40.711926",
    stop_lon: "-73.94067",
    location_type: "1",
    lines_at: ["L"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Canarsie - Rockaway Pkwy",
  },
  L13: {
    complex: false,
    draw: true,
    stop_id: "L13",
    stop_name: "Montrose Av",
    stop_desc: "",
    stop_lat: "40.707739",
    stop_lon: "-73.93985",
    location_type: "1",
    lines_at: ["L"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Canarsie - Rockaway Pkwy",
  },
  L14: {
    complex: false,
    draw: true,
    stop_id: "L14",
    stop_name: "Morgan Av",
    stop_desc: "",
    stop_lat: "40.706152",
    stop_lon: "-73.933147",
    location_type: "1",
    lines_at: ["L"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Canarsie - Rockaway Pkwy",
  },
  L15: {
    complex: false,
    draw: true,
    stop_id: "L15",
    stop_name: "Jefferson St",
    stop_desc: "",
    stop_lat: "40.706607",
    stop_lon: "-73.922913",
    location_type: "1",
    lines_at: ["L"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Canarsie - Rockaway Pkwy",
  },
  L16: {
    complex: false,
    draw: true,
    stop_id: "L16",
    stop_name: "DeKalb Av",
    stop_desc: "",
    stop_lat: "40.703811",
    stop_lon: "-73.918425",
    location_type: "1",
    lines_at: ["L"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Canarsie - Rockaway Pkwy",
  },
  L17: {
    complex: false,
    draw: false,
    stop_id: "L17",
    stop_name: "Myrtle - Wyckoff Avs",
    stop_desc: "",
    stop_lat: "40.699814",
    stop_lon: "-73.911586",
    location_type: "1",
    lines_at: ["L"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "Canarsie - Rockaway Pkwy",
  },
  L19: {
    complex: false,
    draw: true,
    stop_id: "L19",
    stop_name: "Halsey St",
    stop_desc: "",
    stop_lat: "40.695602",
    stop_lon: "-73.904084",
    location_type: "1",
    lines_at: ["L"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Canarsie - Rockaway Pkwy",
  },
  L20: {
    complex: false,
    draw: true,
    stop_id: "L20",
    stop_name: "Wilson Av",
    stop_desc: "",
    stop_lat: "40.688764",
    stop_lon: "-73.904046",
    location_type: "1",
    lines_at: ["L"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "Canarsie - Rockaway Pkwy",
  },
  L21: {
    complex: false,
    draw: true,
    stop_id: "L21",
    stop_name: "Bushwick Av - Aberdeen St",
    stop_desc: "",
    stop_lat: "40.682829",
    stop_lon: "-73.905249",
    location_type: "1",
    lines_at: ["L"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Canarsie - Rockaway Pkwy",
  },
  L22: {
    complex: false,
    draw: false,
    stop_id: "L22",
    stop_name: "Broadway Jct",
    stop_desc: "",
    stop_lat: "40.678856",
    stop_lon: "-73.90324",
    location_type: "1",
    lines_at: ["L"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Canarsie - Rockaway Pkwy",
  },
  L24: {
    complex: false,
    draw: true,
    stop_id: "L24",
    stop_name: "Atlantic Av",
    stop_desc: "",
    stop_lat: "40.675345",
    stop_lon: "-73.903097",
    location_type: "1",
    lines_at: ["L"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Canarsie - Rockaway Pkwy",
  },
  L25: {
    complex: false,
    draw: true,
    stop_id: "L25",
    stop_name: "Sutter Av",
    stop_desc: "",
    stop_lat: "40.669367",
    stop_lon: "-73.901975",
    location_type: "1",
    lines_at: ["L"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Canarsie - Rockaway Pkwy",
  },
  L26: {
    complex: false,
    draw: true,
    stop_id: "L26",
    stop_name: "Livonia Av",
    stop_desc: "",
    stop_lat: "40.664038",
    stop_lon: "-73.900571",
    location_type: "1",
    lines_at: ["L"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Canarsie - Rockaway Pkwy",
  },
  L27: {
    complex: false,
    draw: true,
    stop_id: "L27",
    stop_name: "New Lots Av",
    stop_desc: "",
    stop_lat: "40.658733",
    stop_lon: "-73.899232",
    location_type: "1",
    lines_at: ["L"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Canarsie - Rockaway Pkwy",
  },
  L28: {
    complex: false,
    draw: true,
    stop_id: "L28",
    stop_name: "E 105 St",
    stop_desc: "",
    stop_lat: "40.650573",
    stop_lon: "-73.899485",
    location_type: "1",
    lines_at: ["L"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Canarsie - Rockaway Pkwy",
  },
  L29: {
    complex: false,
    draw: true,
    stop_id: "L29",
    stop_name: "Canarsie - Rockaway Pkwy",
    stop_desc: "",
    stop_lat: "40.646654",
    stop_lon: "-73.90185",
    location_type: "1",
    lines_at: ["L"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "",
  },
  M01: {
    complex: false,
    draw: true,
    stop_id: "M01",
    stop_name: "Middle Village - Metropolitan Av",
    stop_desc: "",
    stop_lat: "40.711396",
    stop_lon: "-73.889601",
    location_type: "1",
    lines_at: ["M"],
    borough: "queens",
    accessible: true,
    north_label: "Manhattan",
    south_label: "",
  },
  M04: {
    complex: false,
    draw: true,
    stop_id: "M04",
    stop_name: "Fresh Pond Rd",
    stop_desc: "",
    stop_lat: "40.706186",
    stop_lon: "-73.895877",
    location_type: "1",
    lines_at: ["M"],
    borough: "queens",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Metropolitan Av",
  },
  M05: {
    complex: false,
    draw: true,
    stop_id: "M05",
    stop_name: "Forest Av",
    stop_desc: "",
    stop_lat: "40.704423",
    stop_lon: "-73.903077",
    location_type: "1",
    lines_at: ["M"],
    borough: "queens",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Metropolitan Av",
  },
  M06: {
    complex: false,
    draw: true,
    stop_id: "M06",
    stop_name: "Seneca Av",
    stop_desc: "",
    stop_lat: "40.702762",
    stop_lon: "-73.90774",
    location_type: "1",
    lines_at: ["M"],
    borough: "queens",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Metropolitan Av",
  },
  M08: {
    complex: false,
    draw: false,
    stop_id: "M08",
    stop_name: "Myrtle - Wyckoff Avs",
    stop_desc: "",
    stop_lat: "40.69943",
    stop_lon: "-73.912385",
    location_type: "1",
    lines_at: ["M"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "Metropolitan Av",
  },
  M09: {
    complex: false,
    draw: true,
    stop_id: "M09",
    stop_name: "Knickerbocker Av",
    stop_desc: "",
    stop_lat: "40.698664",
    stop_lon: "-73.919711",
    location_type: "1",
    lines_at: ["M"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Metropolitan Av",
  },
  M10: {
    complex: false,
    draw: true,
    stop_id: "M10",
    stop_name: "Central Av",
    stop_desc: "",
    stop_lat: "40.697857",
    stop_lon: "-73.927397",
    location_type: "1",
    lines_at: ["M"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Metropolitan Av",
  },
  M11: {
    complex: false,
    draw: true,
    stop_id: "M11",
    stop_name: "Myrtle Av",
    stop_desc: "",
    stop_lat: "40.697207",
    stop_lon: "-73.935657",
    location_type: "1",
    lines_at: ["M", "J", "Z"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Jamaica - Middle Village",
    south_label: "Manhattan",
  },
  M12: {
    complex: false,
    draw: true,
    stop_id: "M12",
    stop_name: "Flushing Av",
    stop_desc: "",
    stop_lat: "40.70026",
    stop_lon: "-73.941126",
    location_type: "1",
    lines_at: ["M", "J"],
    borough: "queens",
    accessible: true,
    north_label: "Jamaica - Middle Village",
    south_label: "Manhattan",
  },
  M13: {
    complex: false,
    draw: true,
    stop_id: "M13",
    stop_name: "Lorimer St",
    stop_desc: "",
    stop_lat: "40.703869",
    stop_lon: "-73.947408",
    location_type: "1",
    lines_at: ["M", "J"],
    borough: "queens",
    accessible: false,
    north_label: "Jamaica - Middle Village",
    south_label: "Manhattan",
  },
  M14: {
    complex: false,
    draw: true,
    stop_id: "M14",
    stop_name: "Hewes St",
    stop_desc: "",
    stop_lat: "40.70687",
    stop_lon: "-73.953431",
    location_type: "1",
    lines_at: ["M", "J"],
    borough: "queens",
    accessible: false,
    north_label: "Jamaica - Middle Village",
    south_label: "Manhattan",
  },
  M16: {
    complex: false,
    draw: true,
    stop_id: "M16",
    stop_name: "Marcy Av",
    stop_desc: "",
    stop_lat: "40.708359",
    stop_lon: "-73.957757",
    location_type: "1",
    lines_at: ["M", "J", "Z"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Jamaica - Middle Village",
    south_label: "Manhattan",
  },
  M18: {
    complex: false,
    draw: false,
    stop_id: "M18",
    stop_name: "Essex St",
    stop_desc: "Delancey St - Essex St",
    stop_lat: "40.718315",
    stop_lon: "-73.987437",
    location_type: "1",
    lines_at: ["J", "Z"],
    borough: "manhattan",
    accessible: false,
    north_label: "Brooklyn",
    south_label: "Broad St / Uptown",
  },
  M19: {
    complex: false,
    draw: true,
    stop_id: "M19",
    stop_name: "Bowery",
    stop_desc: "",
    stop_lat: "40.72028",
    stop_lon: "-73.993915",
    location_type: "1",
    lines_at: ["J", "Z"],
    borough: "manhattan",
    accessible: false,
    north_label: "Brooklyn",
    south_label: "Broad St",
  },
  M20: {
    complex: false,
    draw: false,
    stop_id: "M20",
    stop_name: "Canal St",
    stop_desc: "",
    stop_lat: "40.718092",
    stop_lon: "-73.999892",
    location_type: "1",
    lines_at: ["J", "Z"],
    borough: "manhattan",
    accessible: false,
    north_label: "Brooklyn",
    south_label: "Broad St",
  },
  M21: {
    complex: false,
    draw: false,
    stop_id: "M21",
    stop_name: "Chambers St",
    stop_desc: "",
    stop_lat: "40.713243",
    stop_lon: "-74.003401",
    location_type: "1",
    lines_at: ["J", "Z"],
    borough: "manhattan",
    accessible: true,
    north_label: "Brooklyn",
    south_label: "Broad St",
  },
  M22: {
    complex: false,
    draw: false,
    stop_id: "M22",
    stop_name: "Fulton St",
    stop_desc: "",
    stop_lat: "40.710374",
    stop_lon: "-74.007582",
    location_type: "1",
    lines_at: ["J", "Z"],
    borough: "manhattan",
    accessible: true,
    north_label: "Brooklyn",
    south_label: "Broad St",
  },
  M23: {
    complex: false,
    draw: true,
    stop_id: "M23",
    stop_name: "Broad St",
    stop_desc: "",
    stop_lat: "40.706476",
    stop_lon: "-74.011056",
    location_type: "1",
    lines_at: ["J", "Z"],
    borough: "manhattan",
    accessible: false,
    north_label: "Brooklyn",
    south_label: "",
  },
  N02: {
    complex: false,
    draw: true,
    stop_id: "N02",
    stop_name: "8 Av",
    stop_desc: "",
    stop_lat: "40.635064",
    stop_lon: "-74.011719",
    location_type: "1",
    lines_at: ["N"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  N03: {
    complex: false,
    draw: true,
    stop_id: "N03",
    stop_name: "Fort Hamilton Pkwy",
    stop_desc: "",
    stop_lat: "40.631386",
    stop_lon: "-74.005351",
    location_type: "1",
    lines_at: ["N"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  N04: {
    complex: false,
    draw: false,
    stop_id: "N04",
    stop_name: "New Utrecht Av",
    stop_desc: "",
    stop_lat: "40.624842",
    stop_lon: "-73.996353",
    location_type: "1",
    lines_at: ["N"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  N05: {
    complex: false,
    draw: true,
    stop_id: "N05",
    stop_name: "18 Av",
    stop_desc: "",
    stop_lat: "40.620671",
    stop_lon: "-73.990414",
    location_type: "1",
    lines_at: ["N"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  N06: {
    complex: false,
    draw: true,
    stop_id: "N06",
    stop_name: "20 Av",
    stop_desc: "",
    stop_lat: "40.61741",
    stop_lon: "-73.985026",
    location_type: "1",
    lines_at: ["N"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  N07: {
    complex: false,
    draw: true,
    stop_id: "N07",
    stop_name: "Bay Pkwy",
    stop_desc: "",
    stop_lat: "40.611815",
    stop_lon: "-73.981848",
    location_type: "1",
    lines_at: ["N"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  N08: {
    complex: false,
    draw: true,
    stop_id: "N08",
    stop_name: "Kings Hwy",
    stop_desc: "",
    stop_lat: "40.603923",
    stop_lon: "-73.980353",
    location_type: "1",
    lines_at: ["N"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  N09: {
    complex: false,
    draw: true,
    stop_id: "N09",
    stop_name: "Avenue U",
    stop_desc: "",
    stop_lat: "40.597473",
    stop_lon: "-73.979137",
    location_type: "1",
    lines_at: ["N"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  N10: {
    complex: false,
    draw: true,
    stop_id: "N10",
    stop_name: "86 St",
    stop_desc: "",
    stop_lat: "40.592721",
    stop_lon: "-73.97823",
    location_type: "1",
    lines_at: ["N"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  N12: {
    complex: false,
    draw: true,
    stop_id: "N12",
    stop_name: "S.B. Coney Island",
    stop_desc: "Coney Island - Stillwell Av",
    stop_lat: "40.577422",
    stop_lon: "-73.981233",
    location_type: "1",
    lines_at: ["D", "F", "N", "Q"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "Coney Island",
  },
  Q01: {
    complex: false,
    draw: false,
    stop_id: "Q01",
    stop_name: "Canal St",
    stop_desc: "",
    stop_lat: "40.718383",
    stop_lon: "-74.00046",
    location_type: "1",
    lines_at: ["N", "Q"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & Queens",
    south_label: "Downtown & Brooklyn",
  },
  Q03: {
    complex: false,
    draw: true,
    stop_id: "Q03",
    stop_name: "72 St",
    stop_desc: "",
    stop_lat: "40.768799",
    stop_lon: "-73.958424",
    location_type: "1",
    lines_at: ["Q"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown",
    south_label: "Downtown & Brooklyn",
  },
  Q04: {
    complex: false,
    draw: true,
    stop_id: "Q04",
    stop_name: "86 St",
    stop_desc: "",
    stop_lat: "40.777891",
    stop_lon: "-73.951787",
    location_type: "1",
    lines_at: ["Q"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown",
    south_label: "Downtown & Brooklyn",
  },
  Q05: {
    complex: false,
    draw: true,
    stop_id: "Q05",
    stop_name: "96 St",
    stop_desc: "",
    stop_lat: "40.784318",
    stop_lon: "-73.947152",
    location_type: "1",
    lines_at: ["Q"],
    borough: "manhattan",
    accessible: true,
    north_label: "",
    south_label: "Downtown & Brooklyn",
  },
  R01: {
    complex: false,
    draw: true,
    stop_id: "R01",
    stop_name: "Astoria - Ditmars Blvd",
    stop_desc: "",
    stop_lat: "40.775036",
    stop_lon: "-73.912034",
    location_type: "1",
    lines_at: ["N", "W"],
    borough: "queens",
    accessible: false,
    north_label: "",
    south_label: "Manhattan",
  },
  R03: {
    complex: false,
    draw: true,
    stop_id: "R03",
    stop_name: "Astoria Blvd",
    stop_desc: "",
    stop_lat: "40.770258",
    stop_lon: "-73.917843",
    location_type: "1",
    lines_at: ["N", "W"],
    borough: "queens",
    accessible: true,
    north_label: "Ditmars Blvd",
    south_label: "Manhattan",
  },
  R04: {
    complex: false,
    draw: true,
    stop_id: "R04",
    stop_name: "30 Av",
    stop_desc: "",
    stop_lat: "40.766779",
    stop_lon: "-73.921479",
    location_type: "1",
    lines_at: ["N", "W"],
    borough: "queens",
    accessible: false,
    north_label: "Astoria - Ditmars Blvd",
    south_label: "Manhattan",
  },
  R05: {
    complex: false,
    draw: true,
    stop_id: "R05",
    stop_name: "Broadway",
    stop_desc: "",
    stop_lat: "40.76182",
    stop_lon: "-73.925508",
    location_type: "1",
    lines_at: ["N", "W"],
    borough: "queens",
    accessible: false,
    north_label: "Astoria - Ditmars Blvd",
    south_label: "Manhattan",
  },
  R06: {
    complex: false,
    draw: true,
    stop_id: "R06",
    stop_name: "36 Av",
    stop_desc: "",
    stop_lat: "40.756804",
    stop_lon: "-73.929575",
    location_type: "1",
    lines_at: ["N", "W"],
    borough: "queens",
    accessible: false,
    north_label: "Astoria - Ditmars Blvd",
    south_label: "Manhattan",
  },
  R08: {
    complex: false,
    draw: true,
    stop_id: "R08",
    stop_name: "39 Av",
    stop_desc: "39 Av - Dutch Kills",
    stop_lat: "40.752882",
    stop_lon: "-73.932755",
    location_type: "1",
    lines_at: ["N", "W"],
    borough: "queens",
    accessible: false,
    north_label: "Astoria - Ditmars Blvd",
    south_label: "Manhattan",
  },
  R09: {
    complex: false,
    draw: true,
    stop_id: "R09",
    stop_name: "Queensboro Plaza",
    stop_desc: "",
    stop_lat: "40.750582",
    stop_lon: "-73.940202",
    location_type: "1",
    lines_at: ["7", "N", "W"],
    borough: "queens",
    accessible: false,
    north_label: "Astoria - Flushing",
    south_label: "Manhattan",
  },
  R11: {
    complex: false,
    draw: false,
    stop_id: "R11",
    stop_name: "Lexington Av/59 St",
    stop_desc: "",
    stop_lat: "40.76266",
    stop_lon: "-73.967258",
    location_type: "1",
    lines_at: ["N", "R", "W"],
    borough: "manhattan",
    accessible: false,
    north_label: "Queens",
    south_label: "Downtown & Brooklyn",
  },
  R13: {
    complex: false,
    draw: true,
    stop_id: "R13",
    stop_name: "5 Av/59 St",
    stop_desc: "",
    stop_lat: "40.764811",
    stop_lon: "-73.973347",
    location_type: "1",
    lines_at: ["N", "R", "W"],
    borough: "manhattan",
    accessible: false,
    north_label: "Queens",
    south_label: "Downtown & Brooklyn",
  },
  R14: {
    complex: false,
    draw: true,
    stop_id: "R14",
    stop_name: "57 St - 7 Av",
    stop_desc: "",
    stop_lat: "40.764664",
    stop_lon: "-73.980658",
    location_type: "1",
    lines_at: ["N", "R", "Q", "W"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & Queens",
    south_label: "Downtown & Brooklyn",
  },
  R15: {
    complex: false,
    draw: true,
    stop_id: "R15",
    stop_name: "49 St",
    stop_desc: "",
    stop_lat: "40.759901",
    stop_lon: "-73.984139",
    location_type: "1",
    lines_at: ["N", "R", "W"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & Queens",
    south_label: "Downtown & Brooklyn",
  },
  R16: {
    complex: false,
    draw: false,
    stop_id: "R16",
    stop_name: "Times Sq - 42 St",
    stop_desc: "",
    stop_lat: "40.754672",
    stop_lon: "-73.986754",
    location_type: "1",
    lines_at: ["N", "Q", "R", "W"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & Queens",
    south_label: "Downtown & Brooklyn",
  },
  R17: {
    complex: false,
    draw: false,
    stop_id: "R17",
    stop_name: "34 St - Herald Sq",
    stop_desc: "",
    stop_lat: "40.749567",
    stop_lon: "-73.98795",
    location_type: "1",
    lines_at: ["N", "Q", "R", "W"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & Queens",
    south_label: "Downtown & Brooklyn",
  },
  R18: {
    complex: false,
    draw: true,
    stop_id: "R18",
    stop_name: "28 St",
    stop_desc: "",
    stop_lat: "40.745494",
    stop_lon: "-73.988691",
    location_type: "1",
    lines_at: ["R", "W"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & Queens",
    south_label: "Downtown & Brooklyn",
  },
  R19: {
    complex: false,
    draw: true,
    stop_id: "R19",
    stop_name: "23 St",
    stop_desc: "",
    stop_lat: "40.741303",
    stop_lon: "-73.989344",
    location_type: "1",
    lines_at: ["R", "W"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & Queens",
    south_label: "Downtown & Brooklyn",
  },
  R20: {
    complex: false,
    draw: false,
    stop_id: "R20",
    stop_name: "14 St - Union Sq",
    stop_desc: "",
    stop_lat: "40.735736",
    stop_lon: "-73.990568",
    location_type: "1",
    lines_at: ["N", "Q", "R", "W"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & Queens",
    south_label: "Downtown & Brooklyn",
  },
  R21: {
    complex: false,
    draw: true,
    stop_id: "R21",
    stop_name: "8 St - NYU",
    stop_desc: "",
    stop_lat: "40.730328",
    stop_lon: "-73.992629",
    location_type: "1",
    lines_at: ["R", "W"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & Queens",
    south_label: "Downtown & Brooklyn",
  },
  R22: {
    complex: false,
    draw: true,
    stop_id: "R22",
    stop_name: "Prince St",
    stop_desc: "",
    stop_lat: "40.724329",
    stop_lon: "-73.997702",
    location_type: "1",
    lines_at: ["R", "W"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & Queens",
    south_label: "Downtown & Brooklyn",
  },
  R23: {
    complex: false,
    draw: false,
    stop_id: "R23",
    stop_name: "Canal St",
    stop_desc: "",
    stop_lat: "40.719527",
    stop_lon: "-74.001775",
    location_type: "1",
    lines_at: ["R", "W",],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & Queens",
    south_label: "Downtown & Brooklyn",
  },
  R24: {
    complex: false,
    draw: true,
    stop_id: "R24",
    stop_name: "City Hall",
    stop_desc: "",
    stop_lat: "40.713282",
    stop_lon: "-74.006978",
    location_type: "1",
    lines_at: ["R", "W"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & Queens",
    south_label: "Downtown & Brooklyn",
  },
  R25: {
    complex: false,
    draw: false,
    stop_id: "R25",
    stop_name: "Cortlandt St",
    stop_desc: "",
    stop_lat: "40.710668",
    stop_lon: "-74.011029",
    location_type: "1",
    lines_at: ["R", "W"],
    borough: "manhattan",
    accessible: true,
    north_label: "Uptown & Queens",
    south_label: "Downtown & Brooklyn",
  },
  R26: {
    complex: false,
    draw: true,
    stop_id: "R26",
    stop_name: "Rector St",
    stop_desc: "",
    stop_lat: "40.70722",
    stop_lon: "-74.013342",
    location_type: "1",
    lines_at: ["R", "W"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & Queens",
    south_label: "Downtown & Brooklyn",
  },
  R27: {
    complex: false,
    draw: false,
    stop_id: "R27",
    stop_name: "Whitehall St",
    stop_desc: "",
    stop_lat: "40.703087",
    stop_lon: "-74.012994",
    location_type: "1",
    lines_at: ["R", "W"],
    borough: "manhattan",
    accessible: false,
    north_label: "Uptown & Queens",
    south_label: "Brooklyn",
  },
  R28: {
    complex: false,
    draw: false,
    stop_id: "R28",
    stop_name: "Court St",
    stop_desc: "",
    stop_lat: "40.6941",
    stop_lon: "-73.991777",
    location_type: "1",
    lines_at: ["R"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Bay Ridge - 95 St",
  },
  R29: {
    complex: false,
    draw: false,
    stop_id: "R29",
    stop_name: "Jay St - MetroTech",
    stop_desc: "",
    stop_lat: "40.69218",
    stop_lon: "-73.985942",
    location_type: "1",
    lines_at: ["R"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "Bay Ridge - 95 St",
  },
  R30: {
    complex: false,
    draw: true,
    stop_id: "R30",
    stop_name: "DeKalb Av",
    stop_desc: "",
    stop_lat: "40.690635",
    stop_lon: "-73.981824",
    location_type: "1",
    lines_at: ["B", "N", "Q", "R"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "Coney Island - Bay Ridge",
  },
  R31: {
    complex: false,
    draw: false,
    stop_id: "R31",
    stop_name: "Atlantic Av - Barclays Ctr",
    stop_desc: "",
    stop_lat: "40.683666",
    stop_lon: "-73.97881",
    location_type: "1",
    lines_at: ["N", "Q", "R"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "Coney Island - Bay Ridge",
  },
  R32: {
    complex: false,
    draw: true,
    stop_id: "R32",
    stop_name: "Union St",
    stop_desc: "",
    stop_lat: "40.677316",
    stop_lon: "-73.98311",
    location_type: "1",
    lines_at: ["N", "R"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Bay Ridge - 95 St",
  },
  R33: {
    complex: false,
    draw: false,
    stop_id: "R33",
    stop_name: "9 St",
    stop_desc: "4 Av - 9 St",
    stop_lat: "40.670847",
    stop_lon: "-73.988302",
    location_type: "1",
    lines_at: ["R"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Bay Ridge - 95 St",
  },
  R34: {
    complex: false,
    draw: true,
    stop_id: "R34",
    stop_name: "Prospect Av",
    stop_desc: "",
    stop_lat: "40.665414",
    stop_lon: "-73.992872",
    location_type: "1",
    lines_at: ["N", "R"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Bay Ridge - 95 St",
  },
  R35: {
    complex: false,
    draw: true,
    stop_id: "R35",
    stop_name: "25 St",
    stop_desc: "",
    stop_lat: "40.660397",
    stop_lon: "-73.998091",
    location_type: "1",
    lines_at: ["N", "R"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Bay Ridge - 95 St",
  },
  R36: {
    complex: false,
    draw: true,
    stop_id: "R36",
    stop_name: "36 St",
    stop_desc: "",
    stop_lat: "40.655144",
    stop_lon: "-74.003549",
    location_type: "1",
    lines_at: ["D", "N", "R"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Coney Island - Bay Ridge",
  },
  R39: {
    complex: false,
    draw: true,
    stop_id: "R39",
    stop_name: "45 St",
    stop_desc: "",
    stop_lat: "40.648939",
    stop_lon: "-74.010006",
    location_type: "1",
    lines_at: ["N", "R"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Bay Ridge - 95 St",
  },
  R40: {
    complex: false,
    draw: true,
    stop_id: "R40",
    stop_name: "53 St",
    stop_desc: "",
    stop_lat: "40.645069",
    stop_lon: "-74.014034",
    location_type: "1",
    lines_at: ["N", "R"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "Bay Ridge - 95 St",
  },
  R41: {
    complex: false,
    draw: true,
    stop_id: "R41",
    stop_name: "59 St",
    stop_desc: "",
    stop_lat: "40.641362",
    stop_lon: "-74.017881",
    location_type: "1",
    lines_at: ["N", "R"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "Coney Island - Bay Ridge",
  },
  R42: {
    complex: false,
    draw: true,
    stop_id: "R42",
    stop_name: "Bay Ridge Av",
    stop_desc: "",
    stop_lat: "40.634967",
    stop_lon: "-74.023377",
    location_type: "1",
    lines_at: ["R"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "95 St",
  },
  R43: {
    complex: false,
    draw: true,
    stop_id: "R43",
    stop_name: "77 St",
    stop_desc: "",
    stop_lat: "40.629742",
    stop_lon: "-74.02551",
    location_type: "1",
    lines_at: ["R"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "95 St",
  },
  R44: {
    complex: false,
    draw: true,
    stop_id: "R44",
    stop_name: "86 St",
    stop_desc: "",
    stop_lat: "40.622687",
    stop_lon: "-74.028398",
    location_type: "1",
    lines_at: ["R"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Manhattan",
    south_label: "95 St",
  },
  R45: {
    complex: false,
    draw: true,
    stop_id: "R45",
    stop_name: "Bay Ridge - 95 St",
    stop_desc: "",
    stop_lat: "40.616622",
    stop_lon: "-74.030876",
    location_type: "1",
    lines_at: ["R"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Manhattan",
    south_label: "",
  },
  S01: {
    complex: false,
    draw: false,
    stop_id: "S01",
    stop_name: "Franklin Av",
    stop_desc: "",
    stop_lat: "40.680596",
    stop_lon: "-73.955827",
    location_type: "1",
    lines_at: ["SF"],
    borough: "brooklyn",
    accessible: false,
    north_label: "",
    south_label: "Prospect Park",
  },
  S03: {
    complex: false,
    draw: true,
    stop_id: "S03",
    stop_name: "Park Pl",
    stop_desc: "",
    stop_lat: "40.674772",
    stop_lon: "-73.957624",
    location_type: "1",
    lines_at: ["SF"],
    borough: "brooklyn",
    accessible: true,
    north_label: "Franklin Av",
    south_label: "Prospect Park",
  },
  S04: {
    complex: false,
    draw: false,
    stop_id: "S04",
    stop_name: "Botanic Garden",
    stop_desc: "",
    stop_lat: "40.670343",
    stop_lon: "-73.959245",
    location_type: "1",
    lines_at: ["SF"],
    borough: "brooklyn",
    accessible: false,
    north_label: "Franklin Av",
    south_label: "Prospect Park",
  },
  S09: {
    complex: false,
    draw: true,
    stop_id: "S09",
    stop_name: "Tottenville",
    stop_desc: "",
    stop_lat: "40.512764",
    stop_lon: "-74.251961",
    location_type: "1",
    lines_at: ["SIR"],
    borough: "statenisland",
    accessible: true,
    north_label: "St George",
    south_label: "",
  },
  // S10: {// this station doesn"t exist??
  //   complex: false,
    draw: true,
  //   stop_id: "S10",
  //   stop_name: "Atlantic",
  //   stop_desc: "",
  //   stop_lat: "40.515401",
  //   stop_lon: "-74.245689",
  //   location_type: "1",
  //   lines_at: ["SIR"],
  //   borough: "statenisland",
  //   accessible: false,
  //
  // },
  S11: {
    complex: false,
    draw: true,
    stop_id: "S11",
    stop_name: "Arthur Kill",
    stop_desc: "",
    stop_lat: "40.516578",
    stop_lon: "-74.242096",
    location_type: "1",
    lines_at: ["SIR"],
    borough: "statenisland",
    accessible: true,
    north_label: "St George",
    south_label: "Tottenville",
  },
  S12: {
    complex: false, // this station doesn"t exist??
    stop_id: "S12",
    stop_name: "Nassau",
    stop_desc: "",
    stop_lat: "40.517812",
    stop_lon: "-74.238373",
    location_type: "1",
    lines_at: ["SIR"],
    borough: "statenisland",
    accessible: false,
    north_label: "St George",
    south_label: "Tottenville",
  },
  S13: {
    complex: false,
    draw: true,
    stop_id: "S13",
    stop_name: "Richmond Valley",
    stop_desc: "",
    stop_lat: "40.519631",
    stop_lon: "-74.229141",
    location_type: "1",
    lines_at: ["SIR"],
    borough: "statenisland",
    accessible: false,
    north_label: "St George",
    south_label: "Tottenville",
  },
  S14: {
    complex: false,
    draw: true,
    stop_id: "S14",
    stop_name: "Pleasant Plains",
    stop_desc: "",
    stop_lat: "40.52241",
    stop_lon: "-74.217847",
    location_type: "1",
    lines_at: ["SIR"],
    borough: "statenisland",
    accessible: false,
    north_label: "St George",
    south_label: "Tottenville",
  },
  S15: {
    complex: false,
    draw: true,
    stop_id: "S15",
    stop_name: "Prince's Bay",
    stop_desc: "",
    stop_lat: "40.525507",
    stop_lon: "-74.200064",
    location_type: "1",
    lines_at: ["SIR"],
    borough: "statenisland",
    accessible: false,
    north_label: "St George",
    south_label: "Tottenville",
  },
  S16: {
    complex: false,
    draw: true,
    stop_id: "S16",
    stop_name: "Huguenot",
    stop_desc: "",
    stop_lat: "40.533674",
    stop_lon: "-74.191794",
    location_type: "1",
    lines_at: ["SIR"],
    borough: "statenisland",
    accessible: false,
    north_label: "St George",
    south_label: "Tottenville",
  },
  S17: {
    complex: false,
    draw: true,
    stop_id: "S17",
    stop_name: "Annadale",
    stop_desc: "",
    stop_lat: "40.54046",
    stop_lon: "-74.178217",
    location_type: "1",
    lines_at: ["SIR"],
    borough: "statenisland",
    accessible: false,
    north_label: "St George",
    south_label: "Tottenville",
  },
  S18: {
    complex: false,
    draw: true,
    stop_id: "S18",
    stop_name: "Eltingville",
    stop_desc: "",
    stop_lat: "40.544601",
    stop_lon: "-74.16457",
    location_type: "1",
    lines_at: ["SIR"],
    borough: "statenisland",
    accessible: false,
    north_label: "St George",
    south_label: "Tottenville",
  },
  S19: {
    complex: false,
    draw: true,
    stop_id: "S19",
    stop_name: "Great Kills",
    stop_desc: "",
    stop_lat: "40.551231",
    stop_lon: "-74.151399",
    location_type: "1",
    lines_at: ["SIR"],
    borough: "statenisland",
    accessible: true,
    north_label: "St George",
    south_label: "Tottenville",
  },
  S20: {
    complex: false,
    draw: true,
    stop_id: "S20",
    stop_name: "Bay Terrace",
    stop_desc: "",
    stop_lat: "40.5564",
    stop_lon: "-74.136907",
    location_type: "1",
    lines_at: ["SIR"],
    borough: "statenisland",
    accessible: false,
    north_label: "St George",
    south_label: "Tottenville",
  },
  S21: {
    complex: false,
    draw: true,
    stop_id: "S21",
    stop_name: "Oakwood Heights",
    stop_desc: "",
    stop_lat: "40.56511",
    stop_lon: "-74.12632",
    location_type: "1",
    lines_at: ["SIR"],
    borough: "statenisland",
    accessible: false,
    north_label: "St George",
    south_label: "Tottenville",
  },
  S22: {
    complex: false,
    draw: true,
    stop_id: "S22",
    stop_name: "New Dorp",
    stop_desc: "",
    stop_lat: "40.57348",
    stop_lon: "-74.11721",
    location_type: "1",
    lines_at: ["SIR"],
    borough: "statenisland",
    accessible: false,
    north_label: "St George",
    south_label: "Tottenville",
  },
  S23: {
    complex: false,
    draw: true,
    stop_id: "S23",
    stop_name: "Grant City",
    stop_desc: "",
    stop_lat: "40.578965",
    stop_lon: "-74.109704",
    location_type: "1",
    lines_at: ["SIR"],
    borough: "statenisland",
    accessible: false,
    north_label: "St George",
    south_label: "Tottenville",
  },
  S24: {
    complex: false,
    draw: true,
    stop_id: "S24",
    stop_name: "Jefferson Av",
    stop_desc: "",
    stop_lat: "40.583591",
    stop_lon: "-74.103338",
    location_type: "1",
    lines_at: ["SIR"],
    borough: "statenisland",
    accessible: false,
    north_label: "St George",
    south_label: "Tottenville",
  },
  S25: {
    complex: false,
    draw: true,
    stop_id: "S25",
    stop_name: "Dongan Hills",
    stop_desc: "",
    stop_lat: "40.588849",
    stop_lon: "-74.09609",
    location_type: "1",
    lines_at: ["SIR"],
    borough: "statenisland",
    accessible: true,
    north_label: "St George",
    south_label: "Tottenville",
  },
  S26: {
    complex: false,
    draw: true,
    stop_id: "S26",
    stop_name: "Old Town",
    stop_desc: "",
    stop_lat: "40.596612",
    stop_lon: "-74.087368",
    location_type: "1",
    lines_at: ["SIR"],
    borough: "statenisland",
    accessible: false,
    north_label: "St George",
    south_label: "Tottenville",
  },
  S27: {
    complex: false,
    draw: true,
    stop_id: "S27",
    stop_name: "Grasmere",
    stop_desc: "",
    stop_lat: "40.603117",
    stop_lon: "-74.084087",
    location_type: "1",
    lines_at: ["SIR"],
    borough: "statenisland",
    accessible: false,
    north_label: "St George",
    south_label: "Tottenville",
  },
  S28: {
    complex: false,
    draw: true,
    stop_id: "S28",
    stop_name: "Clifton",
    stop_desc: "",
    stop_lat: "40.621319",
    stop_lon: "-74.071402",
    location_type: "1",
    lines_at: ["SIR"],
    borough: "statenisland",
    accessible: false,
    north_label: "St George",
    south_label: "Tottenville",
  },
  S29: {
    complex: false,
    draw: true,
    stop_id: "S29",
    stop_name: "Stapleton",
    stop_desc: "",
    stop_lat: "40.627915",
    stop_lon: "-74.075162",
    location_type: "1",
    lines_at: ["SIR"],
    borough: "statenisland",
    accessible: false,
    north_label: "St George",
    south_label: "Tottenville",
  },
  S30: {
    complex: false,
    draw: true,
    stop_id: "S30",
    stop_name: "Tompkinsville",
    stop_desc: "",
    stop_lat: "40.636949",
    stop_lon: "-74.074835",
    location_type: "1",
    lines_at: ["SIR"],
    borough: "statenisland",
    accessible: false,
    north_label: "St George",
    south_label: "Tottenville",
  },
  S31: {
    complex: false,
    draw: true,
    stop_id: "S31",
    stop_name: "St George",
    stop_desc: "",
    stop_lat: "40.643748",
    stop_lon: "-74.073643",
    location_type: "1",
    lines_at: ["SIR"],
    borough: "statenisland",
    accessible: true,
    north_label: "",
    south_label: "Tottenville",
  },
  comp01: {
    complex: true,
    draw: true,
    stop_name: "14 St / 6 Av",
    stop_lat: "40.737335",
    stop_lon: "-73.996786",
    stops_at: ["L02", "D19"],
    lines_at: ["F", "M", "L"],
    borough: "manhattan",
    accessible: false,
  },
  comp02: {
    complex: true,
    draw: true,
    stop_name: "14 St - Union Sq",
    stop_lat: "40.7499197",
    stop_long: "-73.9888527",
    stops_at: ["635", "L03", "R20"],
    lines_at: ["4", "5", "6", "L", "N", "Q", "R", "W"],
    borough: "manhattan",
    accessible: true,
  },
  comp03: {
    complex: true,
    draw: true,
    stop_name: "149 St - Grand Concourse",
    stop_lat: "40.818375",
    stop_lon: "-73.927351",
    stops_at: ["222", "415"],
    lines_at: ["2", "4", "5"],
    borough: "bronx",
    accessible: false,
  },
  comp04: {
    complex: true,
    draw: true,
    stop_name: "161 St - Yankee Stadium",
    stop_lat: "40.827905",
    stop_lon: "-73.925651",
    stops_at: ["414", "D11"],
    lines_at: ["4", "B", "D"],
    borough: "bronx",
    accessible: true,
  },
  comp05: {
    complex: true,
    draw: true,
    stop_name: "168 St - Washington Hts",
    stop_lat: "40.840719",
    stop_lon: "-73.939561",
    stops_at: ["112", "A09"],
    lines_at: ["1", "A", "C"],
    borough: "manhattan",
    accessible: false,
  },
  comp06: {
    complex: true,
    draw: true,
    stop_name: "Court Sq - 23 St",
    stop_lat: "40.747023",
    stop_lon: "-73.945264",
    stops_at: ["719", "F09", "G22"],
    lines_at: ["7", "E", "M"],
    borough: "queens",
    accessible: true,
  },
  comp07: {
    complex: true,
    draw: true,
    stop_name: "34 St - Herald Sq",
    stop_lat: "40.7499197",
    stop_long: "-73.9888527",
    stops_at: ["D17", "R17"],
    lines_at: ["B", "D", "F", "M", "N", "Q", "R", "W"],
    borough: "manhattan",
    accessible: true,
  },
  comp08: {
    complex: true,
    draw: true,
    stop_name: "4 Av-9 St",
    stop_lat: "40.7499197",
    stop_long: "-73.9888527",
    stops_at: ["F23", "R33"],
    lines_at: ["F", "G", "R"],
    borough: "brooklyn",
    accessible: false,
  },
  comp09: {
    complex: true,
    draw: true,
    stop_name: "42 St - Bryant Pk",
    stop_lat: "40.7531825",
    stop_long: "-73.987862",
    stops_at: ["724", "D16"],
    lines_at: ["7", "B", "D", "F", "M"],
    borough: "manhattan",
    accessible: false,
  },
  comp10: {
    complex: true,
    draw: true,
    stop_name: "Grand Central - 42 St",
    stop_lat: "40.7507681",
    stop_long: "-73.9811302",
    stops_at: ["631", "723"], //, "901"],
    lines_at: ["4", "5", "6", "7"],
    borough: "manhattan",
    accessible: true,
  },
  comp11: {
    complex: true,
    draw: true,
    stop_name: "Times Sq - 42 St",
    stop_lat: "40.7545655",
    stop_long: "-73.9895308",
    stops_at: ["127", "725", "A27", "R16"],//, "902"],
    lines_at: ["1", "2", "3", "7", "A", "C", "E", "N", "Q", "R", "W"],
    borough: "manhattan",
    accessible: true,
  },
  comp12: {
    complex: true,
    draw: true,
    stop_name: "Lex Av - 51/53 St",
    stop_lat: "40.757552",
    stop_lon: "-73.969055",
    stops_at: ["630", "F11"],
    lines_at: ["6", "E", "M"],
    borough: "manhattan",
    accessible: true,
  },
  comp13: {
    complex: true,
    draw: true,
    stop_name: "Lex Av - 59 St",
    stop_lat: "40.76266",
    stop_lon: "-73.967258",
    stops_at: ["629", "R11"],
    lines_at: ["4", "5", "6", "N", "R", "W"],
    borough: "manhattan",
    accessible: false,
  },
  comp14: {
    complex: true,
    draw: true,
    stop_name: "59 St - Columbus Circle",
    stop_lat: "40.7686521",
    stop_long: "-73.9836425",
    stops_at: ["125", "A24"],
    lines_at: ["1", "2", "3", "A", "C", "B", "D"],
    borough: "manhattan",
    accessible: true,
  },
  comp15: {
    complex: true,
    draw: true,
    stop_name: "62 St - New Utrecht Av",
    stop_lon: "-73.996353",
    stops_at: ["B16", "N04"],
    lines_at: ["B", "N"],
    borough: "brooklyn",
    accessible: true,
  },
  comp16: {
    complex: true,
    draw: true,
    stop_name: "Jackson Hts - Roosevelt Av",
    stop_lat: "40.746739",
    stop_long: "-73.8931873",
    stops_at: ["710", "G14"],
    lines_at: ["7", "E", "F", "M", "R"],
    borough: "queens",
    accessible: true,
  },
  comp17: {
    complex: true,
    draw: true,
    stop_name: "Atlantic Av - Barclays Ctr",
    stop_lat: "40.6845237",
    stop_long: "-73.9778809",
    stops_at: ["235", "D24", "R31"],
    lines_at: ["2", "3", "4", "5", "B", "D", "N", "Q", "R"],
    borough: "brooklyn",
    accessible: true,
  },
  comp18: {
    complex: true,
    draw: true,
    stop_name: "14 St - 8 Av",
    stop_lat: "40.7400907",
    stop_long: "-74.0026457",
    stops_at: ["A31", "L01"],
    lines_at: ["A", "C", "E", "L"],
    borough: "manhattan",
    accessible: true,
  },
  comp19: {
    complex: true,
    draw: true,
    stop_name: "Broadway-Lafayette St - Bleecker St",
    stop_lat: "40.725297",
    stop_lon: "-73.996204",
    stops_at: ["637", "D21"],
    lines_at: ["6", "B", "D", "F", "M"],
    borough: "Manhattan",
    accessible: true,
  },
  comp20: {
    complex: true,
    draw: true,
    stop_name: "Borough Hall - Court St",
    stop_lat: "40.6935978",
    stop_long: "-73.9924742",
    stops_at: ["232", "423", "R28"],
    lines_at: ["2", "3", "4", "5", "R"],
    borough: "brooklyn",
    accessible: true,
  },
  comp21: {
    complex: true,
    draw: true,
    stop_name: "Broadway Junction",
    stop_lat: "40.6784524",
    stop_long: "-73.903297",
    stops_at: ["A51", "J27", "L22"],
    lines_at: ["A", "C", "J", "Z", "L"],
    borough: "brooklyn",
    accessible: false,
  },
  comp22: {
    complex: true,
    draw: true,
    stop_name: "Brookyn Bridge-City Hall / Chambers St",
    stop_lat: "40.713065",
    stop_lon: "-74.004131",
    stops_at: ["640" , "M21"],
    lines_at: ["4", "5", "6", "J", "Z"],
    borough: "manhattan",
    accessible: true,
  },
  comp23: {
    complex: true,
    draw: true,
    stop_name: "Canal St",
    stop_lat: "40.718914",
    stop_long: "-74.0042176",
    stops_at: ["639", "M20", "Q01", "R23"],
    lines_at: ["6", "N", "Q", "R", "W", "J", "Z"],
    borough: "manhattan",
    accessible: true,
  },
  comp24: {
    complex: true,
    draw: true,
    stop_name: "World Trade Center",
    stop_lat: "40.7126452",
    stop_long: "-74.009898",
    stops_at: ["228", "A36", "E01", "R25"],
    lines_at: ["2", "3", "A", "C", "E", "R", "W"],
    borough: "manhatan",
    accessible: true,
  },
  comp25: {
    complex: true,
    draw: true,
    stop_name: "Delancy St - Essex St",
    stop_lat: "40.718611",
    stop_lon: "-73.988114",
    stops_at: ["F15", "M18"],
    lines_at: ["F", "M", "J", "Z"],
    borough: "manhattan",
    accessible: false,
  },
  comp26: {
    complex: true,
    draw: true,
    stop_name: "Franklin Av - Medgar Evers College",
    stop_lat: "40.670682",
    stop_lon: "-73.958131",
    stops_at: ["239", "S04"],
    lines_at: ["2", "3", "4", "5", "SF"],
    borough: "brooklyn",
    accessible: false,
  },
  comp27: {
    complex: true,
    draw: true,
    stop_name: "Franklin Av",
    stop_lat: "40.68138",
    stop_lon: "-73.956848",
    stops_at: ["A45", "S01"],
    lines_at: ["C", "SF"],
    borough: "brooklyn",
    accessible: true,
  },
  comp28: {
    complex: true,
    draw: true,
    stop_name: "Fulton St",
    stop_lat: "40.7097012",
    stop_long: "-74.0105739",
    stops_at: ["229", "418", "A38", "M22"],
    lines_at: ["2", "3", "4", "5", "A", "C", "J", "Z"],
    borough: "mahattan",
    accessible: true,
  },
  comp29: {
    complex: true,
    draw: true,
    stop_name: "Metropolitan Av - Lorimer St",
    stop_lat: "40.712792",
    stop_lon: "-73.951418",
    stops_at: ["G29", "L10"],
    lines_at: ["G", "L"],
    borough: "brooklyn",
    accessible: false,
  },
  comp29: {
    complex: true,
    draw: true,
    stop_name: "Myrtle-Wycoff Avs",
    stop_lat: "40.699814",
    stop_lon: "-73.911586",
    stops_at: ["L17", "M08"],
    lines_at: ["L", "M"],
    borough: "brooklyn",
    accessible: true,
  },
  comp30: {
    complex: true,
    draw: true,
    stop_name: "Whitehall St - South Ferry",
    stop_lat: "40.703087",
    stop_lon: "-74.012994",
    stops_at: ["142", "R27"],
    lines_at: ["1", "R", "W"],
    borough: "manhattan",
    accessible: true,
  },
  comp36: {
    complex: true,
    draw: true,
    stop_name: "Jay St - MetroTech",
    stop_lat: "40.692338",
    stop_lon: "-73.987342",
    stops_at: ["A41", "R29"],
    lines_at: ["A", "C", "F", "R"],
    borough: "brooklyn",
    accessible: true,
  },
}
